...
async function handle(request: Request, env: Env, path: string) {
  ...
  if (path === 'location' && method === 'POST') {
    ...
    const rewardsToSpawn=(activeRewardCount?.count||0)===0?3:0;
  ...
  if(path==='world'&&method==='GET'){
    const [discoveries,territories,drops,reports,crews,seasons,requests,safeHouses,badges,contracts,heatRow,rewards,tracePoints]=await Promise.all([
