...
async function handle(request: Request, env: Env, path: string) {
  ...
  if(path==='venue-bounties'&&method==='GET'){
    const now=new Date().toISOString();const sessions=await env.DB.prepare(`SELECT b.*,u.username,u.tier FROM venue_bounty_sessions b JOIN users u ON u.id=b.host_user_id WHERE b.status='active' AND b.phase_ends_at>? ORDER BY b.stars DESC,b.phase_ends_at LIMIT 30`).bind(now).all();return json({sessions:sessions.results.map((row:any)=>({...row,remainingSeconds:Math.max(0,Math.ceil((Date.parse(row.phase_ends_at)-Date.now())/1000)}))});
  }
