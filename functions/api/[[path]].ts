...
async function handle(request: Request, env: Env, path: string) {
  ...
  if (path === 'routes' && method === 'POST') {
    ...
    const steps:Array<{instruction:string;name:string;distanceM:number;durationS:number;latitude:number;longitude:number}>=[];
    for(let index=0;index<stops.length;index+=20){const segmentStops=stops.slice(index,index+20),routePath=[start,...segmentStops].map(point=>`${point.longitude},${point.latitude}`).join(';');const routeUrl=`https://router.project-osrm.org/route/v1/driving/${routePath}?overview=full&geometries=geojson&steps=true`;let segment: any;try{const routed=await fetch(routeUrl);if(routed.ok){const data=await routed.json<any>();segment=data.routes?.[0];}}catch{}if(!segment)return json({error:`Route service is unavailable near stop ${index+1}. Try a nearby address or retry in a moment.`},502);distance+=segment.distance;duration+=segment.duration;const points=segment.geometry.coordinates.map(([longitude,latitude]:number[])=>({latitude,longitude}));coordinates.push(...(coordinates.length?points.slice(1):points));for(const leg of segment.legs||[])for(const step of leg.steps||[]){const maneuver=step.maneuver||{},type=String(maneuver.type||'continue'),modifier=String(maneuver.modifier||'').replace('_',' '),road=step.name||step.ref||'';const verb=type==='turn'?`Turn ${modifier||'ahead'}`:type==='arrive'?'Arrive at destination':type==='roundabout'?`At roundabout, take exit ${maneuver.exit||''}`:type==='merge'?`Merge ${modifier}`:type==='on ramp'?'Take entrance':type==='off ramp'?'Take exit':type==='continue'?'Continue straight':`${type} ${modifier}`;steps.push({instruction:`${verb}${road?` onto ${road}`:''}`.trim(),name:road||'UNNAMED ROAD',distanceM:Number(step.distance)||0,durationS:Number(step.duration)||0,latitude:Number(maneuver.location?.[1]),longitude:Number(maneuver.location?.[0])});}start=segmentStops[segmentStops.length-1];}
