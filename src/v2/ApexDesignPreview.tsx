...
// Best-effort push registration for Bounty alerts. A permission denial, a
// missing native module, or a network failure here must never block boot.
const registerForPushNotifications=async()=>{
  if(Platform.OS==='web')return;
  try{
    const existing=await Notifications.getPermissionsAsync();
    let status=existing.status;
    if(status!=='granted'){const requested=await Notifications.requestPermissionsAsync();status=requested.status;}
    if(status!=='granted')return;
    const token=(await Notifications.getExpoPushTokenAsync()).data;
    if(!token)return;
    await cloudflareApi.request('/api/push/register',{method:'POST',body:JSON.stringify({token,platform:Platform.OS})});
  }catch{/* Push registration is supplemental; never surface this to the user. */}
};
...
function bearingBetween(a:{latitude:number;longitude:number},b:{latitude:number;longitude:number}){
  const rad=Math.PI/180,dLng=(b.longitude-a.longitude)*rad;
  const y=Math.sin(dLng)*Math.cos(b.latitude*rad);
  const x=Math.cos(a.latitude*rad)*Math.sin(b.latitude*rad)-Math.sin(a.latitude*rad)*Math.cos(b.latitude*rad)*Math.cos(dLng);
...
function CollapsibleSection({ label, action, defaultOpen = false, children }: { label: string; action?: string; defaultOpen?: boolean; children: React.ReactNode }) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <>
      <Pressable onPress={() => setOpen(value => !value)} style={styles.sectionTitleRow}>
        <Text style={styles.sectionTitle}>{label}</Text>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          {action ? <Text style={styles.sectionAction}>{action}</Text> : null}
          <ChevronRight size={14} color={muted} style={{ transform: [{ rotate: open ? '90deg' : '0deg' }] }} />
        </View>
      </Pressable>
      {open ? children : null}
    </>
...
function RadarMap({
  ...
  // The web map is a static srcDoc; changing that attribute on the iframe
  // reloads its whole document (new Leaflet init, tile refetch) regardless
  // of React's own key/remount. GPS updates every 500ms/1m while driving
  // (liveNetworkStore.ts watchPositionAsync), so baking live `location`
  // straight into the regenerated document was reloading the map up to
  // twice a second during a drive. Fix: bake location into the document only
  // once per iframe lifetime (on first fix, and again after a real remount
  // e.g. a street/satellite mode switch) for the initial camera/marker/fog
  // position, then push every subsequent position into the *already-loaded*
  // page via postMessage, where its own script moves the marker and redraws
  // fog without reloading anything.
  const [bakedLocation,setBakedLocation]=useState(location);
  useEffect(()=>{ if(!bakedLocation&&location) setBakedLocation(location); },[location,bakedLocation]);
  const iframeRef=useRef<HTMLIFrameElement|null>(null);
  useEffect(()=>{
    if(Platform.OS!=='web'||!location)return;
    iframeRef.current?.contentWindow?.postMessage({source:'apex-radar-locate',location:{latitude:location.latitude,longitude:location.longitude},follow:Boolean(followLocation)},'*');
  },[location,followLocation]);
  // Newly discovered fog cells (map_discoveries) ARE persisted server-side as
  // the player drives -- but with the document only regenerating on a mode
  // switch, an already-open map never learned about cells discovered after
  // its initial load: the reveal was saved, the live session just never
  // redrew it. Push the static (non-player) reveal zones the same way
  // location updates are pushed.
  useEffect(()=>{
    if(Platform.OS!=='web')return;
    const staticZones=[
      ...discoveries.map(point=>({...point,radiusM:'radiusM' in point?Number(point.radiusM):650})),
      ...events.map(event=>({latitude:event.latitude,longitude:event.longitude,radiusM:Math.max(1600,event.radiusM*3)})),
      ...drops.filter(drop=>!drop.claimed).map(drop=>({latitude:drop.latitude,longitude:drop.longitude,radiusM:Math.max(850,drop.radius_m*6)})),
      ...rewards.map(reward=>({latitude:reward.latitude,longitude:reward.longitude,radiusM:Math.max(850,reward.radius_m*6)})),
    ];
    iframeRef.current?.contentWindow?.postMessage({source:'apex-radar-zones',zones:staticZones},'*');
  },[discoveries,events,drops,rewards]);
  ...
  if (Platform.OS === 'web') {
    ...
    const revealZones=[...discoveries.map(point=>({...point,radiusM:'radiusM' in point?Number(point.radiusM):650})),...events.map(event=>({latitude:event.latitude,longitude:event.longitude,radiusM:Math.max(1600,event.radiusM*3)})),...drops.filter(drop=>!drop.claimed).map(drop=>({latitude:drop.latitude,longitude:drop.longitude,radiusM:Math.max(850,drop.radius_m*6)})),...rewards.map(reward=>({latitude:reward.latitude,longitude:reward.longitude,radiusM:Math.max(850,reward.radius_m*6)})),...(bakedLocation?[{latitude:bakedLocation.latitude,longitude:bakedLocation.longitude,radiusM:16093.4,breach:true}]:[]),...(revealOrigin?[{latitude:revealOrigin.latitude,longitude:revealOrigin.longitude,radiusM:16093.4,breach:true}]:[])];
    ...
    const center = focus || retainedViewport || bakedLocation || { latitude: 20, longitude: 0 };
    const zoom = focus ? 16 : retainedViewport?.zoom || (bakedLocation ? 14 : 2);
    // Defined once per document regardless of whether we have a fix yet, so
    // the live-location message handler (registered in liveLocationOverlay
    // below) can create/move the same marker without duplicating this HTML.
    const selfMarkerFn=`function ensureSelfMarker(lat,lng){if(window.__apexSelfMarker){window.__apexSelfMarker.setLatLng([lat,lng]);}else{window.__apexSelfMarker=L.marker([lat,lng],{zIndexOffset:1000,icon:L.divIcon({className:'',html:'<div class="self-anchor"><i class="radar-ring r1"></i><i class="radar-ring r2"></i><i class="radar-ring r3"></i><i class="radar-sweep"></i><div class="self-dot"></div><b class="self-label">YOU // ACTIVE DRIVER</b></div>',iconSize:[180,180],iconAnchor:[90,90]})}).addTo(map);}return window.__apexSelfMarker;}`;
    const selfMarker = bakedLocation ? `${selfMarkerFn}ensureSelfMarker(${bakedLocation.latitude},${bakedLocation.longitude});` : selfMarkerFn;
    const liveLocationOverlay=`<script>(function(){window.addEventListener('message',function(e){if(!e.data)return;if(e.data.source==='apex-radar-zones'){if(typeof window.__apexUpdateStaticZones==='function')window.__apexUpdateStaticZones(e.data.zones);return;}if(e.data.source!=='apex-radar-locate')return;var loc=e.data.location;if(!loc||typeof loc.latitude!=='number'||typeof loc.longitude!=='number')return;ensureSelfMarker(loc.latitude,loc.longitude);if(typeof window.__apexUpdateFogZones==='function')window.__apexUpdateFogZones(loc);if(e.data.follow)map.panTo([loc.latitude,loc.longitude]);});})();<\/script>`;
    ...
    return (
      <View style={styles.mapFrame}>
        {React.createElement('iframe', {
          ...
          key: mode,
          ref: iframeRef,
          srcDoc: mapDocument.replace('</body>',worldOverlay.replace('t.unlocked?.16:.28','t.unlocked?0.16:0.28')+canvasFogOverlay+smoothFogOverlay+driveTraceOverlay+rewardReplayOverlay+routeFitOverlay+liveLocationOverlay+'</body>'),
          title: 'Apex Map',
...
function RadarScreen({ onTab }: { onTab: (tab: TabKey) => void }) {
  ...
  useEffect(()=>{
    if(!location||!isDriving||!Number.isFinite(location.heading))return;
    for(const r of reports){
      if(!['fixed_camera','speed_trap','police_sighting'].includes(r.type)||warnedReportsRef.current.has(r.id))continue;
      const distance=metersBetween(location,r);
      if(distance>600)continue;
      if(angleDiff(location.heading,bearingBetween(location,r))>35)continue;
      warnedReportsRef.current.add(r.id);
      playInterfaceSound('error');hapticResult('error');
      Alert.alert(r.type==='police_sighting'?'POLICE AHEAD':r.type==='speed_trap'?'SPEED TRAP AHEAD':'FIXED CAMERA AHEAD',`${Math.round(distance)}M · ${r.note||'Reported by another pilot.'}`);
    }
  },[location,reports,isDriving]);
  ...
  const routeDistanceLabel=(kilometers:number)=>unit==='mph'?(kilometers<.32?`${Math.max(50,Math.round(kilometers*3280.84/50)*50)} FT`:`${(kilometers*.621371).toFixed(kilometers<16?1:0)} MI`):(kilometers<.32?`${Math.max(50,Math.round(kilometers*1000/50)*50)} M`:`${kilometers.toFixed(kilometers<16?1:0)} KM`);
...
function BountyScreen({onTab}:{onTab:(tab:TabKey)=>void}){
  ...
  const [history,setHistory]=useState<any[]>([]);const [spectate,setSpectate]=useState<any|null>(null);
  ...
  const loadHistory=async()=>{try{const data=await cloudflareApi.request<any>('/api/bounty/history');setHistory(data.entries||[]);}catch{/* History is supplemental; a failure here should not block the Bounty screen. */}};
  useEffect(()=>{void load();const timer=setInterval(()=>void load(),5000);return()=>clearInterval(timer);},[]);
  useEffect(()=>{void loadHistory();},[]);
  useEffect(()=>{const timer=setInterval(()=>setClock(Date.now()),1000);return()=>clearInterval(timer);},[]);
  const setMode=async(mode:'world'|'venue')=>{try{await cloudflareApi.request('/api/bounty/mode',{method:'PUT',body:JSON.stringify({mode})});await load();}catch(error){setStatus(error instanceof Error?error.message:'Could not switch Bounty mode.');}};
  const watch=async()=>{if(!world?.id)return;try{const data=await cloudflareApi.request<any>(`/api/bounty/world/${world.id}/spectate`);setSpectate(data.event);}catch(error){setStatus(error instanceof Error?error.message:'Could not open spectator view.');}};
  ...
  const venueMode=settings?.preferredMode==='venue';
...
function WorldScreen({onTab}:{onTab:(tab:TabKey)=>void}){
  ...
  const [reportType,setReportType]=useState<RoadReport['type']>('speed_trap');
  ...
  const directionalTypes:RoadReport['type'][]=['fixed_camera','speed_trap'];
  const submitReport=async()=>{const point=await ensureLocation();if(!point)return Alert.alert('Location required','Lock your GPS before placing a road report.');const heading=directionalTypes.includes(reportType)&&Number.isFinite(point.heading)?point.heading:null;if(await world.report(reportType,reportNote,point.latitude,point.longitude,heading))setReportNote('');};
  ...
  return <ScrollView contentContainerStyle={styles.screenContent} showsVerticalScrollIndicator={false}>
    ...
    <SectionTitle label="COMMUNITY ROAD INTEL" action={`${world.reports.length} ACTIVE`}/><GlassPanel style={styles.worldCard}><View style={styles.reportTypes}>{(['speed_trap','fixed_camera','police_sighting','hazard','closure','dangerous_road'] as const).map(type=><Pressable key={type} onPress={()=>setReportType(type)} style={[styles.reportType,reportType===type&&styles.reportTypeActive]}><Text style={[styles.reportTypeText,reportType===type&&styles.reportTypeTextActive]}>{type.replace('_',' ').toUpperCase()}</Text></Pressable>)}</View>{directionalTypes.includes(reportType)?<Text style={styles.commandMeta}>Facing direction is captured from your current heading automatically.</Text>:null}<TextInput value={reportNote} onChangeText={setReportNote} placeholder="Optional note" placeholderTextColor={muted} style={styles.authInput}/><GlassButton label="PIN AT MY LOCATION" icon={MapPin} onPress={()=>void submitReport()} active/></GlassPanel>{world.reports.slice(0,10).map(r=><View key={r.id} style={styles.worldListRow}><Radio size={18} color={muted}/><View style={styles.commandCopy}><Text style={styles.commandTitle}>{r.type.replace('_',' ').toUpperCase()}</Text><Text style={styles.commandMeta}>{r.note||'No note'} · {r.confirmations} CONFIRMED{r.disputes?` · ${r.disputes} DISPUTED`:''}</Text></View><View style={styles.sheetActions}><GlassButton label="STILL THERE" icon={Check} onPress={()=>void world.confirmReport(r.id)} compact active/><GlassButton label="GONE" icon={X} onPress={()=>void world.disputeReport(r.id)} compact/></View></View>)}
...
function MeetScreen() {
  ...
  const [draft,setDraft]=useState({title:'',location:'',stops:'',description:'',rules:'',startsAt:new Date(Date.now()+3600000).toISOString().slice(0,16),maxAttendees:'',recurrence:'none' as 'none'|'weekly'|'biweekly'});
