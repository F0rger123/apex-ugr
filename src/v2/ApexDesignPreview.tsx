...
function AccessPortal({onUnlock,onSignIn}:{onUnlock:(code:string)=>void;onSignIn:()=>void}){
  ...
  return <View style={[styles.accessPortal,denied&&styles.accessPortalDenied]}>
    <LinearGradient colors={denied?['#170103','#050606','#020302']:['#070A08','#030504','#010201']} style={StyleSheet.absoluteFill}/>
    <View pointerEvents="none" style={styles.accessGrid}>{Array.from({length:9},(_,index)=><View key={index} style={[styles.accessGridLine,{top:`${10+index*10}%`}]}/>)}</View>
    <Animated.View pointerEvents="none" style={[styles.lockBreathingGlow,{opacity:lockPulse.interpolate({inputRange:[0,1],outputRange:[.05,.18]}),transform:[{scale:pulseScale}]}]}/>
    <ScrollView contentContainerStyle={styles.accessStage} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      <View style={styles.accessBrandRow}><ApexLogo/><View style={styles.commandCopy}><Text style={styles.accessBrandName}>APEX UGR</Text><Text style={styles.accessBrandMeta}>INVITATION-ONLY DRIVER NETWORK</Text></View><Text style={styles.accessNode}>NODE 01</Text></View>
      <Animated.View style={[styles.accessGate,{opacity:reveal,transform:[{translateX:shake},{translateY:reveal.interpolate({inputRange:[0,1],outputRange:[12,0]})}]}]}>
        <Animated.View style={[styles.accessSeal,{transform:[{scale:pulseScale}]}]}><View style={styles.accessSealInner}><LockKeyhole size={36} color={denied?'#FF6972':paper} strokeWidth={1.35}/></View><View style={styles.accessSealSignal}/></Animated.View>
        <Text style={styles.accessKicker}>SECURE NETWORK ACCESS</Text>
        <Text style={styles.accessTitle}>Enter your code.</Text>
        <Text style={styles.accessDescription}>Six digits issued by an Apex member. Each slot is verified by the private network.</Text>
        <Pressable accessibilityRole="button" accessibilityLabel="Enter six digit access code" onPress={()=>inputRef.current?.focus()} style={styles.accessCodeRow}>
          {Array.from({length:6},(_,index)=><View key={index} style={[styles.accessCodeSlot,Boolean(code[index])&&styles.accessCodeSlotFilled,denied&&styles.accessCodeSlotDenied]}><Text style={[styles.accessCodeSlotText,denied&&styles.accessCodeSlotTextDenied]}>{code[index]||''}</Text></View>)}
        </Pressable>
        <TextInput ref={inputRef} value={code} onChangeText={value=>{setCode(value.replace(/\D/g,'').slice(0,6));setDenied(false);setStatus('SECURE CHANNEL READY');hapticTick();}} keyboardType="number-pad" inputMode="numeric" maxLength={6} caretHidden style={styles.accessNativeInput} onSubmitEditing={()=>void verify()} autoFocus={Platform.OS==='web'}/>
        <View style={styles.accessStatusRow}><View style={[styles.accessStatusDot,code.length===6&&!denied&&styles.accessStatusDotReady,denied&&styles.accessStatusDotDenied]}/><Text style={[styles.accessStatusText,denied&&styles.accessStatusTextDenied]}>{status}</Text><Text style={styles.accessDigitCount}>{code.length}/6</Text></View>
        <View style={styles.accessKeypad}>{keys.map(key=><Pressable key={key} accessibilityLabel={key==='UNLOCK'?'Unlock network':key==='CLEAR'?'Clear access code':`Enter ${key}`} onPress={()=>enterKey(key)} style={({pressed})=>[styles.accessKey,pressed&&styles.accessKeyPressed,key==='UNLOCK'&&code.length===6&&styles.accessKeyReady]}>{key==='CLEAR'?<RotateCcw size={19} color={muted}/>:key==='UNLOCK'?<><LockKeyhole size={18} color={code.length===6?'#061008':paper}/><Text style={[styles.accessKeyAction,code.length===6&&styles.accessKeyActionReady]}>OPEN</Text></>:<><Text style={styles.accessKeyNumber}>{key}</Text><Text style={styles.accessKeySignal}>·</Text></>}</Pressable>)}</View>
        <Pressable onPress={onSignIn} style={({pressed})=>[styles.accessExisting,pressed&&{opacity:.65}]}><Text style={styles.accessExistingText}>ALREADY A MEMBER? SIGN IN</Text><ChevronRight size={14} color={accent}/></Pressable>
      </Animated.View>
      <View style={styles.accessFooter}><Text style={styles.accessFooterText}>APEX SECURE NETWORK</Text><View style={styles.accessFooterLine}/><Text style={styles.accessFooterText}>CHANNEL ENCRYPTED</Text></View>
    </ScrollView>
  </View>;
}
