...
function RadarMap({
  ...
  // The web map is a static srcDoc regenerated on every render, which forces
  // the iframe to fully reload (new Leaflet init, tile refetch) whenever the
  // baked-in location changes. GPS position updates every 500ms/1m while
  // driving (liveNetworkStore.ts watchPositionAsync), so without this the map
  // would reload up to twice a second during an active drive. Throttle the
  // location actually baked into the document instead -- the marker/fog
  // reveal lag a few seconds behind real position, which is imperceptible,
  // rather than the whole map reloading on every tick.
  const throttledLocationRef=useRef<{latitude:number;longitude:number}|null>(location);
  const throttledLocationStampRef=useRef(0);
  const [throttledLocation,setThrottledLocation]=useState(location);
  useEffect(()=>{
    if(!location)return;
    const previous=throttledLocationRef.current;
    const moved=previous?metersBetween(previous,location):Infinity;
    const now=Date.now();
    if(!previous||moved>25||now-throttledLocationStampRef.current>4000){
      throttledLocationRef.current=location;
      throttledLocationStampRef.current=now;
  ...
  if (Platform.OS === 'web') {
    ...
    const revealZones=[...discoveries.map(point=>({...point,radiusM:'radiusM' in point?Number(point.radiusM):650})),...events.map(event=>({latitude:event.latitude,longitude:event.longitude,radiusM:Math.max(1600,event.radiusM*3)})),...drops.filter(drop=>!drop.claimed).map(drop=>({latitude:drop.latitude,longitude:drop.longitude,radiusM:Math.max(850,drop.radius_m*6)})),...rewards.map(reward=>({latitude:reward.latitude,longitude:reward.longitude,radiusM:Math.max(850,reward.radius_m*6)})),...(throttledLocation?[{latitude:throttledLocation.latitude,longitude:throttledLocation.longitude,radiusM:16093.4,breach:true}]:[]),...(revealOrigin?[{latitude:revealOrigin.latitude,longitude:revealOrigin.longitude,radiusM:16093.4,breach:true}]:[])];
