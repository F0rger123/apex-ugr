...
function RadarMap({
  ...
  if (Platform.OS === 'web') {
    ...
    const liveLocationOverlay=`<script>(function(){window.addEventListener('message',function(e){if(!e.data||e.data.source!=='apex-radar-locate')return;var loc=e.data.location;if(!loc||typeof loc.latitude!=='number'||typeof loc.longitude!=='number')return;ensureSelfMarker(loc.latitude,loc.longitude);if(typeof window.__apexUpdateFogZones==='function')window.__apexUpdateFogZones(loc);if(e.data.follow)map.panTo([loc.latitude,loc.longitude]);});})();<\/script>`;
