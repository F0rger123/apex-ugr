...
import { formatDistance, formatSpeed, normalizeSpeedUnit, speedFromKph, unitLabel } from './utils/units';
import {
  Activity,
  BadgeCheck,
  Bell,
...
pathTabs['/app/race']='race';
pathTabs['/app/leaders']='leaderboard';
pathTabs['/app/leaderboard']='leaderboard';
pathTabs['/app/leaderboards']='leaderboard';
pathTabs['/app/meets']='meets';
pathTabs['/app/social']='feed';
pathTabs['/app/parts']='parts';
function routeState(){
...
function ApexLogo(){
  ...
  return <Animated.View style={[styles.apexLogo,{transform:[{translateX:glitch.interpolate({inputRange:[-1,0,1],outputRange:[-.75,0,.75]})}]}]}><Image source={require('../../assets/apex-brand-mark.png')} resizeMode="contain" style={styles.apexLogoImage}/></Animated.View>;
}
...
function CinematicReadout(){
  ...
  useEffect(()=>{Animated.timing(opacity,{toValue:1,duration:520,useNativeDriver:useNativeAnimations}).start();},[opacity]);
  return <Animated.View style={[styles.cinematicReadout,{opacity}]}><Text style={styles.vaporStory}>APEX NETWORK</Text><Text style={styles.playTransitionLabel}>ENCRYPTED CHANNEL</Text><Text style={styles.playTransitionLabel}>ENTERING THE UNDERGROUND</Text></Animated.View>;
}
...
function PlayTransition({onComplete}:{onComplete:()=>void}){
  ...
  return <View style={styles.playTransition}><Video source={require('./assets/apex-underground-intro.mp4')} style={StyleSheet.absoluteFill} resizeMode={ResizeMode.COVER} shouldPlay isMuted={false} onPlaybackStatusUpdate={status=>{if(status.isLoaded&&status.didJustFinish)onComplete();}}/>
      <LinearGradient pointerEvents="none" colors={['rgba(0,0,0,.28)','rgba(0,0,0,.08)','rgba(0,0,0,.84)']} style={StyleSheet.absoluteFill}/><View style={styles.cinematicTop}><Text style={styles.characterIndex}>APEX NETWORK // SESSION INITIALIZATION</Text><View style={styles.livePill}><View style={styles.liveDot}/><Text style={styles.playTransitionLive}>ENCRYPTED</Text></View></View><View pointerEvents="none" style={styles.cinematicStory}><CinematicReadout/><View style={styles.playTransitionTrack}><Animated.View style={[styles.playTransitionFill,{transform:[{scaleX:progress}]}]}/></View></View><Pressable accessibilityRole="button" onPress={onComplete} style={styles.skipIntroButton}><Text style={styles.skipIntroText}>SKIP INTRO</Text><ChevronRight size={15} color={accent}/></Pressable><Animated.View pointerEvents="none" style={[styles.playTransitionSweep,{transform:[{translateY:progress.interpolate({inputRange:[0,1],outputRange:[-240,860]})}]}]}/></View>;
}
...
function GameLobby({onEnter}:{onEnter:(tab:TabKey)=>void}){
  ...
  if(entering)return <PlayTransition onComplete={()=>onEnter('command')}/>;
...
function usePhase2State(intervalMs=7000){
  const [data,setData]=useState<any|null>(null);const [error,setError]=useState('');
  const load=async()=>{try{setData(await cloudflareApi.request<any>('/api/phase2/state'));setError('');}catch(reason){setError(reason instanceof Error?reason.message:'Underground network unavailable.');}};
  useEffect(()=>{void load();const timer=setInterval(()=>void load(),intervalMs);return()=>clearInterval(timer);},[intervalMs]);
...
function rarityColor(rarity?:string){return({COMMON:'#C9D0CB',UNCOMMON:'#A8E6A1',RARE:'#73C7FF',EPIC:'#C8A2FF',LEGENDARY:'#FFE28A',GHOST:'#76FFE2',CLASSIFIED:'#FFFFFF'} as Record<string,string>)[String(rarity||'COMMON').toUpperCase()]||paper;}
...
function CommandScreen({ onTab, onProfile }: { onTab: (tab: TabKey) => void; onProfile: () => void }) {
  ...
  const { drivers, events, cruises, route, networkStatus, isDriving, distanceKm, maxSpeedKph, weeklyTopSpeedKph, unit, startDrive } = useLiveNetworkStore();
  ...
  const [chestReward,setChestReward]=useState<{gcReward:number;rarity:string;streakDay:number}|null>(null);
  const phase2=usePhase2State(10000);const bountyEvent=phase2.data?.bounty?.event;const bountyWorld=bountyEvent?.status==='open'?bountyEvent:null;
  ...
  const claimChest=async()=>{if(!chest?.available||claimingChest)return;setClaimingChest(true);try{const response=await cloudflareApi.request<{claim:{gcReward:number;rarity:string;streakDay:number}}>('/api/daily-chest/claim',{method:'POST'}),reward=response.claim;setChest({available:false,streakCount:reward.streakDay});setChestReward(reward);setChestStage(4);playInterfaceSound('reward');hapticResult('success');await useContentStore.getState().initialize();}catch(error){setChestError(error instanceof Error?error.message:'Reward service unavailable.');setChestStage(0);}finally{setClaimingChest(false);}};
  const advanceChest=()=>{if(!chest?.available||claimingChest)return;if(chestStage<3){setChestStage(value=>value+1);hapticTick();playInterfaceSound(chestStage===2?'unlock':'toggle');return;}void claimChest();};
  ...
  return (
    <ScrollView contentContainerStyle={styles.screenContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.commandTelemetryRail}><View style={styles.commandTelemetryCell}><Gauge size={15} color={accent}/><Text style={styles.commandTelemetryValue}>{Math.round(speedFromKph(isDriving?maxSpeedKph:weeklyTopSpeedKph,unit))}</Text><Text style={styles.commandTelemetryLabel}>{isDriving?`MAX ${unitLabel(unit)}`:`WEEKLY ${unitLabel(unit)}`}</Text></View><View style={styles.commandTelemetryCell}><Route size={15} color={paper}/><Text style={styles.commandTelemetryValue}>{route?route.stops.length:ghostReplays.length}</Text><Text style={styles.commandTelemetryLabel}>{route?'ROUTE STOPS':'GHOSTS'}</Text></View><View style={styles.commandTelemetryCell}><CircleDollarSign size={15} color={paper}/><Text style={styles.commandTelemetryValue}>{rewards.length}</Text><Text style={styles.commandTelemetryLabel}>LIVE CACHES</Text></View><View style={styles.commandTelemetryCell}><Navigation size={15} color={paper}/><Text style={styles.commandTelemetryValue}>{formatDistance(distanceKm,unit,true).replace(' ','')}</Text><Text style={styles.commandTelemetryLabel}>RUN</Text></View></View>

      <Pressable accessibilityRole="button" onPress={advanceChest} style={[styles.dailyChest, chest?.available&&styles.dailyChestReady, chestStage===4&&styles.dailyChestOpened]}>
        <View style={[styles.dailyChestCore,chestStage>0&&styles.dailyChestCoreActive,chestStage===4&&styles.dailyChestCoreOpened]}><LockKeyhole size={24} color={chestStage===4?accent:paper}/></View>
        <View style={styles.commandCopy}><Text style={styles.commandTitle}>{chestStage===4?'SIGNAL DECRYPTED':chest?.available?'DAILY GHOST CHEST':'DAILY GHOST CHEST SECURED'}</Text><Text style={styles.commandMeta}>{chestStage===4&&chestReward?`+${chestReward.gcReward} GC · ${chestReward.rarity} · DAY ${chestReward.streakDay}`:chest?.available?['SIGNAL FOUND · TAP TO ARM','SECURITY RINGS ACTIVE · TAP TO BREACH','DECRYPTING CHANNEL · TAP TO UNLOCK','FINAL CIPHER · TAP TO REVEAL'][chestStage]:`NEXT SIGNAL // TOMORROW · STREAK ${chest?.streakCount||0}`}</Text>
          {chest?.available&&chestStage<4?<View style={styles.dailyChestProgress}>{[0,1,2,3].map(stage=><View key={stage} style={[styles.dailyChestProgressDot,chestStage>stage&&styles.dailyChestProgressDotActive]}/>)}</View>:null}</View>
        <Gift size={20} color={chest?.available?accent:muted}/>
      </Pressable>
...
function RadarMap({
  ...
  const lastFollowRevisionRef=useRef<number|null>(null);
  ...
  const sourceSignature=`${mode}:${drivers.length}:${events.length}:${routeCoordinates.length}:${routeStops.length}:${rewards.length}:${drops.length}:${reports.length}:${safeHouses.length}:${discoveries.length}`;
  ...
  useEffect(() => {
    ...
    const listener = (event: MessageEvent) => {
      ...
      if (event.data.viewport && !followLocationRef.current) {
        const viewport=event.data.viewport;
        if(Number.isFinite(viewport.latitude)&&Number.isFinite(viewport.longitude)&&Number.isFinite(viewport.zoom))webViewport.current=viewport;
        return;
      }
  ...
  if (Platform.OS === 'web') {
    ...
    // Leaflet already updates overlays during zoom. A perpetual requestAnimationFrame loop here caused visible mobile redraw churn.
    const smoothFogOverlay=`<script>(function(){map.on('zoomend moveend resize',()=>{});})();<\/script>`;
    ...
    const projectedFogOverlay=`<script>(function(){const zones=${safeDiscoveries},started=performance.now();let overlay=null,raf=0;function escapeSvg(value){return String(value).replace(/[&<>"]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[char]));}function radiusPixels(d,zoom){const lat=Number(d.latitude),lng=Number(d.longitude),meters=Number(d.radiusM||650),east=lng+meters/(111320*Math.max(.2,Math.cos(lat*Math.PI/180))),a=map.project([lat,lng],zoom),b=map.project([lat,east],zoom);return Math.max(6,Math.abs(b.x-a.x));}function render(){raf=0;const bounds=map.getBounds(),zoom=map.getZoom(),nw=map.project(bounds.getNorthWest(),zoom),se=map.project(bounds.getSouthEast(),zoom),width=Math.max(1,se.x-nw.x),height=Math.max(1,se.y-nw.y),age=Math.min(1,Math.max(0,(performance.now()-started)/900)),ease=1-Math.pow(1-age,3);let holes='';for(const d of zones){const p=map.project([Number(d.latitude),Number(d.longitude)],zoom),r=radiusPixels(d,zoom)*(d.breach?(.72+.28*ease):1),x=p.x-nw.x,y=p.y-nw.y;holes+='<circle cx=\"'+x.toFixed(1)+'\" cy=\"'+y.toFixed(1)+'\" r=\"'+r.toFixed(1)+'\" fill=\"black\"/>';if(d.breach&&age<1)holes+='<circle cx=\"'+x.toFixed(1)+'\" cy=\"'+y.toFixed(1)+'\" r=\"'+(r*1.03).toFixed(1)+'\" fill=\"none\" stroke=\"rgba(184,255,169,'+(1-age).toFixed(2)+')\" stroke-width=\"2\"/>';}let wisps='';for(let i=0;i<18;i++){const x=((i*173)%Math.max(1,width+180))-90,y=((i*97)%Math.max(1,height+140))-70,r=95+(i%5)*34;wisps+='<circle cx=\"'+x+'\" cy=\"'+y+'\" r=\"'+r+'\" fill=\"rgba(190,198,193,.08)\"/>';}const svg='<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"'+width+'\" height=\"'+height+'\" viewBox=\"0 0 '+width+' '+height+'\"><defs><mask id=\"fog-cut\"><rect width=\"100%\" height=\"100%\" fill=\"white\"/>'+holes+'</mask></defs><rect width=\"100%\" height=\"100%\" fill=\"rgba(99,106,102,.64)\" mask=\"url(#fog-cut)\"/><g mask=\"url(#fog-cut)\">'+wisps+'</g></svg>';const url='data:image/svg+xml;charset=utf-8,'+encodeURIComponent(svg);if(!overlay){overlay=L.imageOverlay(url,bounds,{pane:'fogPane',interactive:false,opacity:1}).addTo(map);}else{overlay.setUrl(url);overlay.setBounds(bounds);}}function schedule(){if(!raf)raf=requestAnimationFrame(render);}map.createPane('fogPane');map.getPane('fogPane').style.zIndex='480';map.getPane('fogPane').style.pointerEvents='none';map.on('zoomend moveend resize',schedule);map.on('zoomstart movestart',()=>{if(overlay&&overlay.getElement())overlay.getElement().style.transition='none';});render();})();<\/script>`;
    ...
    const routeFitOverlay=`<script>(function(){const route=${safeRoute};if(route.length>1&&${focus||fitAll||ghostReplay||webViewport.current?'false':'true'}){map.fitBounds(L.latLngBounds(route.map(point=>[point.latitude,point.longitude])),{padding:[58,58],maxZoom:16,animate:true,duration:.65});}})();<\/script>`;
    const retainedViewport=!followLocation&& !focus ? webViewport.current : null;
    const center = focus || retainedViewport || location || { latitude: 20, longitude: 0 };
    const zoom = focus ? 16 : retainedViewport?.zoom || (location ? 14 : 2);
  ...
  useEffect(()=>{if(Platform.OS==='web'||!nativeMapRef.current)return;if(!shouldFollowLocation){lastCameraLocation.current=null;lastFollowRevisionRef.current=null;return;}if(!location)return;const previous=lastCameraLocation.current;const moved=!previous||Math.abs(previous.latitude-location.latitude)+Math.abs(previous.longitude-location.longitude)>.00012;if(!moved&&lastFollowRevisionRef.current===followRevision)return;const firstLock=lastFollowRevisionRef.current!==followRevision;lastFollowRevisionRef.current=followRevision;lastCameraLocation.current={latitude:location.latitude,longitude:location.longitude};const camera=firstLock?{center:{latitude:location.latitude,longitude:location.longitude},pitch:42,heading:location.heading||0,zoom:16}:{center:{latitude:location.latitude,longitude:location.longitude},heading:location.heading||0};nativeMapRef.current.animateCamera(camera,{duration:firstLock?650:420});},[shouldFollowLocation,followRevision,location?.latitude,location?.longitude,location?.heading]);
  return (
    <View style={styles.mapFrame}>
      <NativeMap
        ref={nativeMapRef}
...
function RadarScreen({ onTab }: { onTab: (tab: TabKey) => void }) {
  const phase2=usePhase2State(7000);const bountyEvent=phase2.data?.bounty?.event;const worldBounty=bountyEvent?.status==='open'?bountyEvent:null;const frequency=phase2.data?.progress?.frequency;
  ...
  const humanDrivers = liveDrivers.map((driver: LiveDriver): Driver => ({
    ...
    rank: driver.tier, distance: formatSpeed(driver.speedKph,unit), mystery: driver.mystery,
  ...
  const recenter = async () => {
    if(followUser){setFollowUser(false);playInterfaceSound('toggle');return;}
    await lockLocation();
    setSelected(null);
  ...
  const setFrequency=async(level:number)=>{try{await cloudflareApi.request('/api/ghost/frequency',{method:'PUT',body:JSON.stringify({level})});playInterfaceSound('unlock');await phase2.reload();}catch(error){Alert.alert('Frequency locked',error instanceof Error?error.message:'This channel is classified.');}};
  ...
  const routeDistanceLabel=(kilometers:number)=>formatDistance(kilometers,unit);
  ...
  return (
    <View style={styles.radarScreen}>
      ...
      <View style={styles.radarTopControls}>
        ...
        <Pressable onPress={recenter} style={[styles.locateButton,followUser&&styles.locateButtonActive]}><Crosshair size={16} color={followUser?'#041006':accent} /><Text style={[styles.locateText,followUser&&styles.locateTextActive]}>{followUser?'LOCKED ON':'UNLOCKED'}</Text></Pressable>
      </View>
      <Pressable accessibilityLabel="Show all pilots" onPress={()=>{playInterfaceSound('toggle');setFollowUser(false);setSelected(null);setSelectedEvent(null);setSelectedReward(null);setSelectedCrewId(null);setSelectedReplayId(null);setFitAll(true);setFollowRevision(value=>value+1);}} style={styles.networkPill}><Maximize2 size={14} color={accent}/><Text style={styles.networkPillText}>{drivers.filter(item=>item.isLive).length} LIVE</Text></Pressable>
...
function FeedScreen({onTab}:{onTab:(tab:TabKey)=>void}) {
  ...
  const [feedPageHeight,setFeedPageHeight]=useState(Math.max(420,Dimensions.get('window').height-310));
  ...
  return (
    <View style={styles.feedScreen}>
      ...
      {userId&&posts.length?<View style={styles.feedListViewport} onLayout={event=>{const height=Math.round(event.nativeEvent.layout.height);if(height>=420&&height!==feedPageHeight)setFeedPageHeight(height);}}><FlatList data={posts} keyExtractor={post=>post.id} showsVerticalScrollIndicator={false} pagingEnabled snapToInterval={feedPageHeight} decelerationRate="fast" directionalLockEnabled nestedScrollEnabled keyboardShouldPersistTaps="handled" initialNumToRender={1} maxToRenderPerBatch={1} windowSize={3} removeClippedSubviews={Platform.OS!=='web'} getItemLayout={(_,index)=>({length:feedPageHeight,offset:feedPageHeight*index,index})} onMomentumScrollEnd={event=>setActiveIndex(Math.round(event.nativeEvent.contentOffset.y/feedPageHeight))} renderItem={({item:post,index})=><View style={[styles.feedPage,{height:feedPageHeight}]}>
...
function FeedVideo({uri,active,muted}:{uri:string;active:boolean;muted:boolean}){
  ...
  const [status,setStatus]=useState({loaded:false,playing:false,position:0,duration:0,finished:false,error:false});
  ...
  useEffect(()=>()=>{void video.current?.unloadAsync().catch(()=>undefined);},[]);
  const update=(next:AVPlaybackStatus)=>{
    if(!next.isLoaded){setStatus(current=>({...current,loaded:false,playing:false,error:Boolean(next.error)}));return;}
    setStatus({loaded:true,playing:next.isPlaying,position:next.positionMillis,duration:next.durationMillis||0,finished:Boolean(next.didJustFinish),error:false});
  };
  ...
  return <View style={StyleSheet.absoluteFill}>
    ...
    {status.error?<Pressable onPress={()=>void video.current?.replayAsync()} style={styles.feedVideoRetry}><RotateCcw size={18} color={paper}/><Text style={styles.feedVideoRetryText}>RETRY VIDEO</Text></Pressable>:null}
...
function BountyScreen({onTab}:{onTab:(tab:TabKey)=>void}){
  ...
  const phase2=usePhase2State(7000);const world=phase2.data?.bounty?.event;const worldOpen=world?.status==='open';
  ...
  const load=async()=>{setBusy(true);try{const [settingsData,activeData,wantedData,statsData,disclaimer]=await Promise.all([cloudflareApi.request<any>('/api/bounty/settings'),cloudflareApi.request<any>('/api/bounty/active'),cloudflareApi.request<any>('/api/bounty/most-wanted'),cloudflareApi.request<any>('/api/bounty/stats'),cloudflareApi.request<any>('/api/disclaimer')]);setSettings(settingsData.settings);setActive(activeData);setMostWanted(wantedData.mostWanted||[]);setStats(statsData.stats);setAccepted(Boolean(disclaimer.accepted));setStatus('');}catch(error){setStatus(error instanceof Error?error.message:'Bounty network unavailable.');}finally{setBusy(false);}};
  useEffect(()=>{void load();const timer=setInterval(()=>void load(),5000);return()=>clearInterval(timer);},[]);
  useEffect(()=>{const timer=setInterval(()=>setClock(Date.now()),1000);return()=>clearInterval(timer);},[]);
  const saveOptIn=async(enabled:boolean)=>{if(enabled&&!accepted){setStatus('ACCEPT THE SAFETY AGREEMENT FIRST');return;}setBusy(true);try{if(enabled)await cloudflareApi.request('/api/disclaimer/accept',{method:'POST'});await cloudflareApi.request('/api/bounty/settings',{method:'PUT',body:JSON.stringify({bountyModeEnabled:enabled,agreed:accepted,notificationsEnabled:true,showPublicPhoto:true,allowMostWanted:true})});await load();setStatus(enabled?'BOUNTY NETWORK ARMED':'BOUNTY NETWORK DISARMED');}catch(error){setStatus(error instanceof Error?error.message:'Could not update Bounty Mode.');}finally{setBusy(false);}};
  ...
  const worldAction=async(action:'accept'|'decline'|'pulse'|'capture')=>{if(!world?.id)return;setBusy(true);try{const point=useLiveNetworkStore.getState().location;const result=await cloudflareApi.request<any>(`/api/bounty/world/${world.id}/${action}`,{method:'POST',body:JSON.stringify(point?{latitude:point.latitude,longitude:point.longitude,accuracyMeters:point.accuracy}: {})});if(result.capture)setStatus(result.capture.locking?`CAPTURE SIGNAL LOCKING // ${result.capture.seconds}/${result.capture.requiredSeconds}`:'SIGNAL LOST');else if(result.captured)setStatus(`TARGET CAPTURED // +${result.rewardGc} GC`);else setStatus(action==='accept'?'CONTRACT ACCEPTED':action==='decline'?'CONTRACT DECLINED':'SIGNAL UPDATED');playInterfaceSound(result.captured?'reward':'toggle');await phase2.reload();}catch(error){setStatus(error instanceof Error?error.message:'World Bounty update failed.');}finally{setBusy(false);}};
  useEffect(()=>{if(!worldOpen||world?.role!=='hunter'||!world?.id)return;const timer=setInterval(async()=>{try{const point=useLiveNetworkStore.getState().location;await cloudflareApi.request(`/api/bounty/world/${world.id}/pulse`,{method:'POST',body:JSON.stringify(point?{latitude:point.latitude,longitude:point.longitude,accuracyMeters:point.accuracy}: {})});await phase2.reload();}catch{}},7000);return()=>clearInterval(timer);},[worldOpen,world?.role,world?.id]);
  const session=active?.session,role=active?.role as 'target'|'hunter'|null,stageEnd=Date.parse(session?.stageEndsAt||''),remaining=Number.isFinite(stageEnd)?Math.max(0,Math.ceil((stageEnd-clock)/1000)):Math.max(0,Number(session?.remainingSeconds||0));
  return <ScrollView contentContainerStyle={styles.screenContent} showsVerticalScrollIndicator={false}><View style={styles.raceHeader}><View><Text style={styles.eyebrow}>MAP PROTOCOL</Text><Text style={styles.feedTitle}>BOUNTY</Text><Text style={styles.moreCopy}>Opt-in world signals and authorized private sessions. Approximate human intelligence only.</Text></View><Pressable onPress={()=>onTab('radar')} style={styles.iconButton}><Map size={18} color={paper}/></Pressable></View>
  <GlassPanel glow={Boolean(worldOpen)} style={worldOpen&&world?.starLevel===5?styles.mostWantedPanel:undefined}><View style={styles.bountyTop}><View><Text style={styles.identityLabel}>{worldOpen&&world?.starLevel===5?'MOST WANTED':'GENERAL BOUNTY NETWORK'}</Text><Text style={styles.commandTitle}>{world?`${'★'.repeat(world.starLevel)}${'☆'.repeat(5-world.starLevel)} // ${world.status.toUpperCase()}`:'NEXT BOUNTY SIGNAL'}</Text></View><View style={[styles.bountyBadge,Boolean(worldOpen)&&styles.bountyBadgeLive]}><Radio size={14} color={worldOpen?accent:muted}/><Text style={styles.bountyBadgeText}>{worldOpen?'LIVE':world?'RESOLVED':'ARMED'}</Text></View></View>{world?<><View style={styles.bountyWorldMetrics}><View><Text style={styles.bountyReward}>{world.rewardGc} GC</Text><Text style={styles.driverStatLabel}>{worldOpen?'LIVE REWARD':'FINAL REWARD'}</Text></View><View><Text style={styles.bountyHeat}>{worldOpen?world.heat:0}%</Text><Text style={styles.driverStatLabel}>HEAT</Text></View><View><Text style={styles.bountyTimer}>{worldOpen?countdownLabel(world.endsAt):'00:00:00'}</Text><Text style={styles.driverStatLabel}>{worldOpen?'SERVER TIMER':'CLOSED'}</Text></View></View><Text style={styles.commandMeta}>{!worldOpen?`EVENT RESOLVED // ${world.status.toUpperCase()} // NEXT WINDOW ${countdownLabel(phase2.data?.bounty?.nextAt)}`:world.blackout?'GHOST NETWORK BLACKOUT // NAVIGATION REMAINS ONLINE':world.role==='hunter'?`${world.hunterSignal?.direction||'SEARCH'} · ${world.hunterSignal?.approxDistanceMiles||'—'} MI · SIGNAL ${world.hunterSignal?.strength||0}% · AUTO-REFRESH 7S`:world.role==='target'?world.nearestHunter?`HUNTER APPROACHING // ${world.nearestHunter.direction} // ${world.nearestHunter.approxDistanceMiles} MI`:`${world.hunters.length} HUNTER SIGNALS`:world.offer==='pending'?'A TARGET HAS ENTERED THE NETWORK // ACCEPT CONTRACT?':`${world.target?.actorType==='npc'?'NPC TARGET':'HUMAN TARGET'} // ${world.target?.vehicleLabel||'VEHICLE UNKNOWN'}`}</Text>{worldOpen&&world.offer==='pending'?<View style={styles.sheetActions}><GlassButton label="ACCEPT" icon={Crosshair} onPress={()=>void worldAction('accept')} active grow/><GlassButton label="DECLINE" icon={X} onPress={()=>void worldAction('decline')} grow/></View>:worldOpen&&world.role==='hunter'?<View style={styles.sheetActions}><GlassButton label="REFRESH SIGNAL" icon={Radio} onPress={()=>void worldAction('pulse')} active grow/><GlassButton label="CAPTURE LOCK" icon={Crosshair} onPress={()=>void worldAction('capture')} grow/></View>:null}</>:<Text style={styles.commandMeta}>SERVER WINDOW // {countdownLabel(phase2.data?.bounty?.nextAt)}</Text>}</GlassPanel>
  <SectionTitle label="PRIVATE VENUE SESSION" action="OPTIONAL"/><GlassPanel glow={Boolean(session)}><View style={styles.bountyTop}><View><Text style={styles.identityLabel}>NETWORK STATUS</Text><Text style={styles.commandTitle}>{session?`${role?.toUpperCase()} // ${'★'.repeat(session.starLevel)}`:settings?.bountyModeEnabled?'ARMED':'OPTED OUT'}</Text></View><View style={[styles.bountyBadge,settings?.bountyModeEnabled&&styles.bountyBadgeLive]}><Radio size={14} color={settings?.bountyModeEnabled?accent:muted}/><Text style={styles.bountyBadgeText}>{busy?'SYNC':'LIVE'}</Text></View></View>{session?<><Text style={styles.bountyTimer}>{Math.floor(remaining/60)}:{String(remaining%60).padStart(2,'0')}</Text><Text style={styles.commandMeta}>{role==='hunter'?`${session.approxDistanceMiles??'—'} MI · ${session.approxDirection||'SEARCH ZONE PENDING'} · SIGNAL ${session.signalStrengthPct||0}%`:`SURVIVE AUTHORIZED SESSION · ${session.rewardGc} GC`}</Text><View style={styles.sheetActions}>{role==='hunter'?<><GlassButton label="SYNC SIGNAL" icon={Radio} onPress={()=>void sessionAction('signal')} active grow/><GlassButton label="CLAIM" icon={BadgeCheck} onPress={()=>void sessionAction('claim')} grow/><GlassButton label="LEAVE" icon={X} onPress={()=>void sessionAction('leave')} grow/></>:<GlassButton label="SYNC PHASE" icon={Star} onPress={()=>void sessionAction('progress')} active grow/>}</View></>:<><Pressable onPress={()=>setAccepted(value=>!value)} style={styles.bountyConsent}><View style={[styles.bountyCheck,accepted&&styles.bountyCheckOn]}>{accepted?<Check size={13} color="#071009"/>:null}</View><Text style={styles.bountyConsentText}>I accept responsibility and will participate only in safe, legal activity while obeying traffic rules and venue requirements.</Text></Pressable><View style={styles.sheetActions}><GlassButton label={settings?.bountyModeEnabled?'DISABLE':'ENABLE'} icon={ShieldCheck} onPress={()=>void saveOptIn(!settings?.bountyModeEnabled)} active={!settings?.bountyModeEnabled} grow/><GlassButton label="SETTINGS" icon={Settings} onPress={()=>onTab('settings')} grow/></View>{settings?.bountyModeEnabled?<><TextInput value={venueName} onChangeText={setVenueName} placeholder="Authorized venue name" placeholderTextColor={muted} style={styles.authInput}/><GlassButton label={isDriving?'BEGIN PRIVATE SESSION':'START DRIVE + BEGIN'} icon={Play} onPress={()=>void begin()} active/></>:null}</>}</GlassPanel>{status?<Text style={status.includes('ACCEPTED')||status.includes('CAPTURED')||status.includes('ARMED')||status.includes('UPDATED')?styles.healthGood:styles.networkError}>{status}</Text>:null}<SectionTitle label="MOST WANTED" action={`${mostWanted.length+(worldOpen&&world?.starLevel===5?1:0)} ACTIVE`}/>{mostWanted.length?mostWanted.map(item=><GlassPanel key={item.id}><View style={styles.healthRow}><View style={styles.commandCopy}><Text style={styles.commandTitle}>{item.targetUsername?.toUpperCase()||'UNKNOWN PILOT'} · {'★'.repeat(item.starLevel)}</Text><Text style={styles.commandMeta}>{item.targetRank} · {item.rewardGc} GC · {Math.ceil(item.remainingSeconds/60)} MIN</Text></View><GlassButton label="TRACK" icon={Crosshair} onPress={()=>void join(item.id)} compact active/></View></GlassPanel>):<GlassPanel style={styles.emptyState}><Radio size={25} color={muted}/><Text style={styles.emptyTitle}>NO PRIVATE SIGNALS</Text><Text style={styles.emptyCopy}>General network events remain playable through explicit NPC actors when active human population is low.</Text></GlassPanel>}<SectionTitle label="PILOT RECORD"/><GlassPanel><View style={styles.driverStats}><View><Text style={styles.driverStatValue}>{stats?.successfulClaims||0}</Text><Text style={styles.driverStatLabel}>CLAIMS</Text></View><View><Text style={styles.driverStatValue}>{stats?.escapes||0}</Text><Text style={styles.driverStatLabel}>ESCAPES</Text></View><View><Text style={styles.driverStatValue}>{Math.max(stats?.highestStarClaimed||0,stats?.highestStarSurvived||0)}</Text><Text style={styles.driverStatLabel}>MAX STAR</Text></View></View></GlassPanel></ScrollView>;
}
...
function LegacyWorldScreen({onTab}:{onTab:(tab:TabKey)=>void}){
  ...
  const phase2=usePhase2State(10000);
  ...
  const trailAction=async(trail:any)=>{try{if(trail.status==='active'&&trail.stage?.latitude!=null){await setRouteToPoint(`${trail.title} // ${trail.stage.label}`,trail.stage.latitude,trail.stage.longitude);onTab('radar');return;}const result=await cloudflareApi.request<any>(`/api/ghost/trails/${trail.id}/progress`,{method:'POST'});setPhaseStatus(result.completed?`SIGNAL DECRYPTED // +${result.rewardGc} GC // +1 GHOST KEY`:result.started?'TRAIL ANCHOR SECURED':'STAGE DECRYPTED');playInterfaceSound(result.completed?'reward':'unlock');await phase2.reload();}catch(error){setPhaseStatus(error instanceof Error?error.message:'Trail signal failed.');}};
  const decryptTrail=async(trail:any)=>{try{const result=await cloudflareApi.request<any>(`/api/ghost/trails/${trail.id}/progress`,{method:'POST'});setPhaseStatus(result.completed?`SIGNAL DECRYPTED // +${result.rewardGc} GC // +1 GHOST KEY`:'STAGE DECRYPTED');playInterfaceSound(result.completed?'reward':'unlock');await phase2.reload();}catch(error){setPhaseStatus(error instanceof Error?error.message:'Move inside the signal radius first.');}};
  ...
  <SectionTitle label="GHOST TRAILS" action={`FREQ ${String(phase2.data?.progress?.frequency?.activeLevel||1).padStart(2,'0')}`}/>{(phase2.data?.progress?.trails||[]).map((trail:any)=><GlassPanel key={trail.id} glow={trail.status==='active'}><View style={styles.healthRow}><View style={styles.commandCopy}><Text style={styles.commandTitle}>{trail.title}</Text><Text style={styles.commandMeta}>{trail.status==='completed'?'SIGNAL BROKEN':trail.status==='active'?`${trail.stage?.label||'SIGNAL'} · ${trail.currentStage+1}/${trail.totalStages} · ${trail.stage?.radiusMeters||80}M DECRYPT ZONE`:`${trail.totalStages} STAGES · ${trail.rewardGc} GC · FREQ ${String(trail.requiredFrequency).padStart(2,'0')}`}</Text></View><Text style={[styles.contractReward,{color:trail.eligible?accent:muted}]}>{trail.status==='completed'?'CLEARED':trail.eligible?'LIVE':'LOCKED'}</Text></View>{trail.status!=='completed'?<View style={styles.sheetActions}><GlassButton label={trail.status==='active'?'PIN STAGE':'START TRAIL'} icon={trail.status==='active'?Navigation:Radio} onPress={()=>void trailAction(trail)} active={trail.eligible} grow/>{trail.status==='active'?<GlassButton label="DECRYPT" icon={LockKeyhole} onPress={()=>void decryptTrail(trail)} grow/>:null}</View>:null}</GlassPanel>)}{phaseStatus?<Text style={phaseStatus.includes('outside')||phaseStatus.includes('inside')||phaseStatus.includes('failed')?styles.networkError:styles.healthGood}>{phaseStatus}</Text>:null}
  <SectionTitle label="DISTRICTS" action={`${(phase2.data?.progress?.districts||[]).filter((item:any)=>item.state==='MASTERED').length} MASTERED`}/>{(phase2.data?.progress?.districts||[]).length?(phase2.data.progress.districts as any[]).map((district:any)=><View key={district.id} style={styles.worldListRow}><Map size={20} color={district.state==='MASTERED'?accent:paper}/><View style={styles.commandCopy}><Text style={styles.commandTitle}>{district.name}</Text><Text style={styles.commandMeta}>{district.state} · {district.roadsExplored}/{district.totalRoadCells} ROAD CELLS</Text><View style={styles.contractTrack}><View style={[styles.contractFill,{width:`${district.percent}%`}]}/></View></View><Text style={styles.dropValue}>{district.percent}%</Text></View>):<GlassPanel style={styles.emptyState}><Map size={22} color={muted}/><Text style={styles.emptyTitle}>NO DISTRICT SIGNAL</Text><Text style={styles.emptyCopy}>Drive with exploration active to reveal the first district.</Text></GlassPanel>}
  <SectionTitle label="THE LIST" action="REGIONAL"/>{(phase2.data?.progress?.list||[]).slice(0,5).map((pilot:any,index:number)=><View key={pilot.id} style={styles.worldListRow}><Text style={styles.featuredBadgeSlot}>{index+1}</Text><View style={styles.commandCopy}><Text style={styles.commandTitle}>{pilot.display_name||pilot.username}</Text><Text style={styles.commandMeta}>{String(pilot.tier).toUpperCase()} · {pilot.points} REP · {pilot.bounty_claims+pilot.bounty_escapes} BOUNTY RESULTS</Text></View></View>)}
  <SectionTitle label="RANK TRIALS" action="SERVER VERIFIED"/>{(phase2.data?.progress?.trials||[]).map((trial:any)=><View key={trial.rank} style={styles.worldListRow}><ShieldCheck size={20} color={trial.eligible?accent:muted}/><View style={styles.commandCopy}><Text style={styles.commandTitle}>{trial.rank} TRIAL</Text><Text style={styles.commandMeta}>{Object.entries(trial.requirements).map(([key,value])=>`${String(key).toUpperCase()} ${value}`).join(' · ')}</Text></View><Text style={trial.eligible?styles.healthGood:styles.commandMeta}>{trial.eligible?'ELIGIBLE':'LOCKED'}</Text></View>)}
...
function WorldScreen({onTab}:{onTab:(tab:TabKey)=>void}){
  ...
  const trailAction=async(trail:any)=>{if(!trail.eligible){setPhaseStatus(`REQUIRES FREQ ${String(trail.requiredFrequency).padStart(2,'0')}`);return;}try{if(trail.status==='active'&&trail.stage?.latitude!=null){await setRouteToPoint(trail.stage.label,trail.stage.latitude,trail.stage.longitude);onTab('radar');return;}const point=await ensureLocation();if(!point){setPhaseStatus('LOCK A FRESH LOCATION FIRST');return;}const result=await cloudflareApi.request<any>(`/api/ghost/trails/${trail.id}/progress`,{method:'POST'});setPhaseStatus(result.started?'TRAIL ANCHOR SECURED':'TRAIL SIGNAL UPDATED');await phase2.reload();}catch(error){setPhaseStatus(error instanceof Error?error.message:'TRAIL SIGNAL FAILED');}};
  const decryptTrail=async(trail:any)=>{try{const result=await cloudflareApi.request<any>(`/api/ghost/trails/${trail.id}/progress`,{method:'POST'});setPhaseStatus(result.completed?`TRAIL CLEARED // +${result.rewardGc} GC +1 KEY`:result.advanced?'NEXT SIGNAL DECRYPTED':'SIGNAL UPDATED');await phase2.reload();}catch(error){setPhaseStatus(error instanceof Error?error.message:'SIGNAL OUTSIDE DECRYPT RANGE');}};
  return <ScrollView contentContainerStyle={styles.screenContent} showsVerticalScrollIndicator={false}>
    ...
    <SectionTitle label="GHOST TRAILS" action={`FREQ ${String(phase2.data?.progress?.frequency?.activeLevel||1).padStart(2,'0')}`}/>{(phase2.data?.progress?.trails||[]).map((trail:any)=><GlassPanel key={trail.id} glow={trail.status==='active'}><View style={styles.healthRow}><View style={styles.commandCopy}><Text style={styles.commandTitle}>{trail.title}</Text><Text style={styles.commandMeta}>{trail.status==='completed'?'SIGNAL BROKEN':trail.status==='active'?`${trail.stage?.label||'SIGNAL'} · ${trail.currentStage+1}/${trail.totalStages} · ${trail.stage?.radiusMeters||80}M DECRYPT ZONE`:`${trail.totalStages} STAGES · ${trail.rewardGc} GC · FREQ ${String(trail.requiredFrequency).padStart(2,'0')}`}</Text></View><Text style={[styles.contractReward,{color:trail.eligible?accent:muted}]}>{trail.status==='completed'?'CLEARED':trail.eligible?'LIVE':'LOCKED'}</Text></View>{trail.status!=='completed'?<View style={styles.sheetActions}><GlassButton label={trail.status==='active'?'PIN STAGE':'START TRAIL'} icon={trail.status==='active'?Navigation:Radio} onPress={()=>void trailAction(trail)} active={trail.eligible} grow/>{trail.status==='active'?<GlassButton label="DECRYPT" icon={LockKeyhole} onPress={()=>void decryptTrail(trail)} grow/>:null}</View>:null}</GlassPanel>)}{phaseStatus?<Text style={phaseStatus.includes('FAILED')||phaseStatus.includes('OUTSIDE')||phaseStatus.includes('REQUIRES')?styles.networkError:styles.healthGood}>{phaseStatus}</Text>:null}
    <SectionTitle label="DISTRICT CONTROL" action="DRIVE TO REVEAL"/>{(phase2.data?.progress?.districts||[]).length?(phase2.data?.progress?.districts||[]).map((district:any)=><View key={district.id} style={styles.worldListRow}><Map size={20} color={district.state==='MASTERED'?accent:muted}/><View style={styles.commandCopy}><Text style={styles.commandTitle}>{district.name}</Text><Text style={styles.commandMeta}>{district.state} · {district.roadsExplored}/{district.totalRoadCells} ROAD CELLS</Text><View style={styles.contractTrack}><View style={[styles.contractFill,{width:`${district.percent}%`}]}/></View></View><Text style={styles.dropValue}>{district.percent}%</Text></View>):<GlassPanel style={styles.emptyState}><Map size={22} color={muted}/><Text style={styles.emptyTitle}>NO DISTRICT SIGNAL</Text><Text style={styles.emptyCopy}>Drive with exploration active to reveal the first district.</Text></GlassPanel>}
    <SectionTitle label="THE LIST" action="REGIONAL"/>{(phase2.data?.progress?.list||[]).slice(0,5).map((pilot:any,index:number)=><View key={pilot.id} style={styles.worldListRow}><Text style={styles.featuredBadgeSlot}>{index+1}</Text><View style={styles.commandCopy}><Text style={styles.commandTitle}>{pilot.display_name||pilot.username}</Text><Text style={styles.commandMeta}>{String(pilot.tier).toUpperCase()} · {pilot.points} REP · {pilot.bounty_claims+pilot.bounty_escapes} BOUNTY RESULTS</Text></View></View>)}
    <SectionTitle label="RANK TRIALS" action="SERVER VERIFIED"/>{(phase2.data?.progress?.trials||[]).map((trial:any)=><View key={trial.rank} style={styles.worldListRow}><ShieldCheck size={20} color={trial.eligible?accent:muted}/><View style={styles.commandCopy}><Text style={styles.commandTitle}>{trial.rank} TRIAL</Text><Text style={styles.commandMeta}>{Object.entries(trial.requirements).map(([key,value])=>`${String(key).toUpperCase()} ${value}`).join(' · ')}</Text></View><Text style={trial.eligible?styles.healthGood:styles.commandMeta}>{trial.eligible?'ELIGIBLE':'LOCKED'}</Text></View>)}
    <SectionTitle label="ENCRYPTED CONTRACTS" action={`${world.contracts.filter(item=>item.progress_status==='completed').length} CLEARED`}/>{world.contracts.map(contract=><GlassPanel key={contract.id} style={styles.contractCard} glow={contract.progress_status==='completed'}><View style={styles.worldCardTop}><View style={styles.commandCopy}><Text style={styles.commandTitle}>{contract.title}</Text><Text style={styles.commandMeta}>{contract.description}</Text></View><Text style={styles.contractReward}>+{contract.reward_credits} ACR</Text></View><View style={styles.contractTrack}><View style={[styles.contractFill,{width:`${Math.min(100,Math.round(((contract.progress||0)/contract.target)*100))}%`}]}/></View><View style={styles.contractFooter}><Text style={styles.commandMeta}>{contract.progress||0} / {contract.target}</Text>{!contract.progress_status?<GlassButton label="DECRYPT" icon={LockKeyhole} onPress={()=>void world.acceptContract(contract.id)} compact/>:<Text style={contract.progress_status==='completed'?styles.contractComplete:styles.contractActive}>{contract.progress_status.toUpperCase()}</Text>}</View></GlassPanel>)}
...
function GarageScreen({ onTab }: { onTab: (tab: TabKey) => void }) {
  ...
  const [modDraft,setModDraft]=useState({part:'',brand:'',category:'Performance',price:'',priority:'MEDIUM',notes:'',url:'',installed:false,installedAt:'',sortOrder:0});
  const [modStatus,setModStatus]=useState('');
  const [photoUri, setPhotoUri] = useState<string | null>(null);
  const [vehicleDraft, setVehicleDraft] = useState({ nickname: '', year: String(new Date().getFullYear()), make: '', model: '', trim: 'Base', engine: 'Stock', drivetrain: 'AWD', horsepower: '300', color: 'Black', vehicleType:'CAR' as 'CAR'|'MOTORCYCLE', displacementCc:'' });
  ...
  useEffect(()=>{vehicles.forEach(vehicle=>{if(vehicle.photoUrl)void Image.prefetch(vehicle.photoUrl).catch(()=>undefined);if(vehicle.digitalTwinUrl)void Image.prefetch(vehicle.digitalTwinUrl).catch(()=>undefined);});},[vehicles]);
  ...
  const loadMods=async()=>{if(!activeVehicleId){setMods([]);return;}try{const data=await cloudflareApi.request<{wishlist:any[]}>(`/api/vehicles/${activeVehicleId}/wishlist`);setMods(data.wishlist||[]);setModStatus('SAVED TO YOUR GARAGE');}catch(error){setModStatus(error instanceof Error?error.message:'BUILD PLANNER UNAVAILABLE');}};
  useEffect(()=>{void loadMods();},[activeVehicleId]);
  const saveMod=async()=>{if(!activeVehicleId||!modDraft.part.trim())return;try{await cloudflareApi.request(`/api/vehicles/${activeVehicleId}/wishlist`,{method:'POST',body:JSON.stringify({...modDraft,price:Number(modDraft.price)||0,installedAt:modDraft.installed?modDraft.installedAt||new Date().toISOString():null})});setModDraft({part:'',brand:'',category:'Performance',price:'',priority:'MEDIUM',notes:'',url:'',installed:false,installedAt:'',sortOrder:0});playInterfaceSound('unlock');await loadMods();}catch(error){setModStatus(error instanceof Error?error.message:'BUILD PLANNER SAVE FAILED');}};
  const updateMod=async(item:any, patch:Record<string,unknown>)=>{if(!activeVehicleId)return;try{await cloudflareApi.request(`/api/vehicles/${activeVehicleId}/wishlist`,{method:'POST',body:JSON.stringify({...item,...patch,installedAt:patch.installed?(item.installed_at||new Date().toISOString()):null})});await loadMods();}catch(error){setModStatus(error instanceof Error?error.message:'BUILD PLANNER UPDATE FAILED');}};
  ...
  const saveVehicle = async () => {
    const ready = await addVehicle({ ...vehicleDraft, nickname: vehicleDraft.nickname || `${vehicleDraft.make} ${vehicleDraft.model}`, year: Number(vehicleDraft.year), horsepower: Number(vehicleDraft.horsepower) || 0, displacementCc:Number(vehicleDraft.displacementCc)||0 }, photoUri);
    if (ready) { setShowAdd(false); setPhotoUri(null); setVehicleDraft({ nickname: '', year: String(new Date().getFullYear()), make: '', model: '', trim: 'Base', engine: 'Stock', drivetrain: 'AWD', horsepower: '300', color: 'Black',vehicleType:'CAR',displacementCc:'' }); }
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.screenContent} showsVerticalScrollIndicator={false}>
      <View style={styles.garageSwitcher}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.garageVehicleRail}>
        {vehicles.map((item,index) => (
          <Pressable key={item.id} onPress={() => setActiveVehicle(item.id)} style={[styles.carChip, activeVehicleId === item.id && styles.carChipActive]}>
            ...
            <Text numberOfLines={1} style={[styles.carChipText, activeVehicleId === item.id && styles.carChipTextActive]}>{item.nickname.toUpperCase()}</Text><Text style={styles.carChipOrdinal}>{index+1}/{vehicles.length}</Text>
          </Pressable>
        ))}
        </ScrollView>
      ...
      {(showAdd || !car) ? <GlassPanel style={styles.vehicleForm} glow><View style={styles.notificationHeader}><View><Text style={styles.eyebrow}>VERIFIED FITMENT IDENTITY</Text><Text style={styles.notificationTitle}>ADD VEHICLE</Text></View>{car ? <Pressable onPress={() => setShowAdd(false)} style={styles.closeButton}><X size={17} color={paper} /></Pressable> : null}</View><Pressable onPress={pickVehiclePhoto} style={styles.vehiclePhotoPicker}>{photoUri ? <Image source={{ uri: photoUri }} style={styles.vehiclePhotoPreview} /> : <><CarFront size={32} color={accent} /><Text style={styles.composerHint}>UPLOAD YOUR ACTUAL CAR PHOTO</Text></>}</Pressable>
        <View style={styles.fitmentStep}><Text style={styles.fitmentStepNo}>01</Text><View style={styles.commandCopy}><Text style={styles.commandTitle}>VEHICLE IDENTITY</Text><Text style={styles.commandMeta}>NHTSA make and model catalog</Text></View></View><View style={styles.sheetActions}>{(['CAR','MOTORCYCLE'] as const).map(type=><Pressable key={type} onPress={()=>setVehicleDraft(current=>({...current,vehicleType:type,drivetrain:type==='MOTORCYCLE'?'CHAIN':'AWD'}))} style={[styles.durationChoice,vehicleDraft.vehicleType===type&&styles.durationChoiceActive]}><Text style={[styles.segmentText,vehicleDraft.vehicleType===type&&styles.segmentTextActive]}>{type}</Text></Pressable>)}</View>
        ...
        <View style={styles.choiceSection}><Text style={styles.identityLabel}>ENGINE / POWER UNIT</Text><View style={styles.choiceRail}>{(vehicleDraft.vehicleType==='MOTORCYCLE'?['Single','Parallel Twin','V-Twin','Triple','Inline-4','Flat Twin','Electric']:['Stock','I3','I4','I5','I6','Flat-4','Flat-6','V6','V8','V10','V12','W12','Rotary','Diesel','Hybrid','Plug-in Hybrid','Single Motor EV','Dual Motor EV','Tri Motor EV']).map(item=><Pressable key={item} onPress={()=>updateVehicle('engine',item)} style={[styles.choiceChip,vehicleDraft.engine===item&&styles.choiceChipActive]}><Text style={[styles.choiceChipText,vehicleDraft.engine===item&&styles.choiceChipTextActive]}>{item.toUpperCase()}</Text></Pressable>)}</View></View>
        <View style={styles.choiceSection}><Text style={styles.identityLabel}>{vehicleDraft.vehicleType==='MOTORCYCLE'?'FINAL DRIVE':'DRIVETRAIN'}</Text><View style={styles.choiceRail}>{(vehicleDraft.vehicleType==='MOTORCYCLE'?['CHAIN','BELT','SHAFT','DIRECT ELECTRIC']:['FWD','RWD','AWD','4WD','4x4','Dual Motor AWD','Tri Motor AWD']).map(item=><Pressable key={item} onPress={()=>updateVehicle('drivetrain',item)} style={[styles.choiceChip,vehicleDraft.drivetrain===item&&styles.choiceChipActive]}><Text style={[styles.choiceChipText,vehicleDraft.drivetrain===item&&styles.choiceChipTextActive]}>{item}</Text></Pressable>)}</View></View>
        {vehicleDraft.vehicleType==='MOTORCYCLE'?<View style={styles.catalogFieldWide}><Text style={styles.identityLabel}>DISPLACEMENT (CC)</Text><TextInput value={vehicleDraft.displacementCc} onChangeText={value=>updateVehicle('displacementCc',value.replace(/\D/g,'').slice(0,5))} keyboardType="numeric" placeholder="EX. 1000" placeholderTextColor={muted} style={styles.authInput}/></View>:null}
      ...
      <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} decelerationRate="fast" scrollEventThrottle={16} onMomentumScrollEnd={event=>{const index=Math.round(event.nativeEvent.contentOffset.x/Math.max(1,screenWidth-32));const selected=vehicles[index];if(selected&&selected.id!==activeVehicleId)setActiveVehicle(selected.id);}} style={styles.garageCarousel}>
      {vehicles.map(item=><View key={item.id} style={styles.garageCarouselPage}><View style={styles.garageHero}>{item.photoUrl ? <Image source={{ uri: item.photoUrl }} style={styles.garageImage} /> : <View style={[styles.garageImage, styles.productImageMissing]}><CarFront size={54} color={muted} /></View>}
        <LinearGradient colors={['transparent', 'rgba(2,3,2,0.95)']} style={StyleSheet.absoluteFill} />
        <View style={styles.garageHeroCopy}>
          <View><Text style={styles.eyebrow}>{item.id===activeVehicleId?'ACTIVE BUILD':'SWIPE TO SELECT'}</Text><Text style={styles.garageName}>{item.nickname.toUpperCase()}</Text><Text style={styles.garageModel}>{item.year} {item.make} {item.model} {item.trim || ''}</Text></View>
          <BadgeCheck size={28} color={accent} fill="rgba(145,185,133,.12)" />
        </View>
      </View></View>)}</ScrollView>
      {vehicles.length>1?<View style={styles.garageDots}>{vehicles.map(item=><View key={item.id} style={[styles.garageDot,item.id===activeVehicleId&&styles.garageDotActive]}/>)}</View>:null}
      ...
      <SectionTitle label="BUILD PLANNER" action={`${mods.length} PARTS`} />
      <GlassPanel glow><View style={styles.fitmentBanner}><RefreshCw size={19} color={accent}/><View style={styles.commandCopy}><Text style={styles.commandTitle}>CURRENT + PLANNED BUILD</Text><Text style={styles.commandMeta}>{modStatus||'PERSISTENT PARTS FOR THIS VEHICLE'}</Text></View></View><TextInput value={modDraft.part} onChangeText={part=>setModDraft(current=>({...current,part}))} placeholder="Part name" placeholderTextColor={muted} style={styles.authInput}/><View style={styles.sheetActions}><TextInput value={modDraft.brand} onChangeText={brand=>setModDraft(current=>({...current,brand}))} placeholder="Brand" placeholderTextColor={muted} style={[styles.authInput,{flex:1}]}/><TextInput value={modDraft.price} onChangeText={price=>setModDraft(current=>({...current,price:price.replace(/[^0-9.]/g,'')}))} keyboardType="decimal-pad" placeholder="Est. cost" placeholderTextColor={muted} style={[styles.authInput,{flex:1}]}/></View><View style={styles.sheetActions}><TextInput value={modDraft.category} onChangeText={category=>setModDraft(current=>({...current,category}))} placeholder="Category" placeholderTextColor={muted} style={[styles.authInput,{flex:1}]}/><TextInput value={modDraft.url} onChangeText={url=>setModDraft(current=>({...current,url}))} placeholder="Part link" autoCapitalize="none" placeholderTextColor={muted} style={[styles.authInput,{flex:1}]}/></View><TextInput value={modDraft.notes} onChangeText={notes=>setModDraft(current=>({...current,notes}))} placeholder="Notes" placeholderTextColor={muted} style={styles.authInput}/><View style={styles.sheetActions}>{(['LOW','MEDIUM','HIGH'] as const).map(priority=><Pressable key={priority} onPress={()=>setModDraft(current=>({...current,priority}))} style={[styles.durationChoice,modDraft.priority===priority&&styles.durationChoiceActive]}><Text style={[styles.segmentText,modDraft.priority===priority&&styles.segmentTextActive]}>{priority}</Text></Pressable>)}</View><GlassButton label="ADD PLANNED PART" icon={Plus} onPress={()=>void saveMod()} active/></GlassPanel>
      {mods.length?<View style={styles.buildTotals}><Text style={styles.identityLabel}>BUILD COST</Text><Text style={styles.buildTotalValue}>{mods.filter(item=>item.installed).reduce((sum,item)=>sum+Number(item.price||0),0).toLocaleString(undefined,{style:'currency',currency:'USD'})} INSTALLED · {mods.filter(item=>!item.installed).reduce((sum,item)=>sum+Number(item.price||0),0).toLocaleString(undefined,{style:'currency',currency:'USD'})} PLANNED</Text></View>:null}
      {mods.map(item=><View key={item.id} style={styles.worldListRow}><Pressable onPress={()=>void updateMod(item,{installed:!item.installed})} style={styles.productIconButton}><PackageCheck size={19} color={item.installed?accent:paper}/></Pressable><View style={styles.commandCopy}><Text style={styles.commandTitle}>{String(item.part).toUpperCase()}</Text><Text style={styles.commandMeta}>{item.installed?'INSTALLED':'PLANNED'} · {String(item.brand||'UNBRANDED').toUpperCase()} · {item.priority} · {Number(item.price||0).toLocaleString(undefined,{style:'currency',currency:'USD'})}</Text></View><Pressable onPress={()=>void updateMod(item,{sortOrder:Math.max(0,Number(item.sort_order||0)-1)})} style={styles.productIconButton}><ChevronUp size={15} color={muted}/></Pressable><Pressable onPress={()=>void updateMod(item,{sortOrder:Number(item.sort_order||0)+1})} style={styles.productIconButton}><ChevronDown size={15} color={muted}/></Pressable>{item.url?<Pressable onPress={()=>Linking.openURL(item.url)} style={styles.productIconButton}><ExternalLink size={15} color={accent}/></Pressable>:null}<Pressable onPress={()=>void deleteMod(item.id)} style={styles.productIconButton}><X size={15} color={muted}/></Pressable></View>)}
      </>}
    </ScrollView>
  );
}
...
function RaceScreen() {
  ...
  const {route,location,unit,toggleUnit}=useLiveNetworkStore();
  ...
  return (
    <ScrollView contentContainerStyle={styles.screenContent} showsVerticalScrollIndicator={false}>
      ...
      <GlassPanel style={styles.speedPanel} glow={tracking}>
        <View style={styles.speedHeader}><Text style={[styles.speedStatus, tracking && { color: accent }]}>{gpsStatus}</Text><Pressable onPress={toggleUnit} style={styles.unitButton}><Text style={styles.unitText}>{unitLabel(unit)}</Text></Pressable><View style={[styles.liveDot, tracking && styles.liveDotBright]} /></View>
        <Text style={styles.speedValue}>{Math.round(speedFromKph(speed,unit))}</Text>
        <Text style={styles.speedUnit}>{unitLabel(unit)}</Text>
        <View style={styles.speedMetaRow}><Text style={styles.speedMeta}>MAX {formatSpeed(maxSpeed,unit)}</Text><Text style={styles.speedMeta}>FORMAT {format}</Text></View>
...
function VaultScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={styles.screenContent} showsVerticalScrollIndicator={false}>
      ...
      <GlassPanel glow><View style={styles.ghostCoinHero}><View style={styles.ghostCoinMetal}><View style={styles.ghostCoinCircuit}/><Text style={styles.ghostCoinMark}>A</Text></View><View style={styles.commandCopy}><Text style={styles.ghostCoinValue}>{Number(ghost?.profile?.credits||0).toLocaleString()} GC</Text><Text style={styles.commandMeta}>ENCRYPTED GHOST CREDIT LEDGER</Text></View><Gem size={23} color={accent}/></View><View style={styles.identityDivider}/><Text style={styles.commandMeta}>GHOST STREAK x{ghost?.profile?.current_streak||0} · BEST x{ghost?.profile?.best_streak||0} · {phase2.data?.progress?.keys||0} GHOST KEYS</Text><View style={styles.streakProgress}><Text style={styles.cacheStatus}>FREQ {String(phase2.data?.progress?.frequency?.activeLevel||1).padStart(2,'0')} // UNLOCKED {String(phase2.data?.progress?.frequency?.unlockedLevel||1).padStart(2,'0')}</Text><Text style={styles.commandMeta}>RANK, CONTRACTS, AND GHOST STREAK CONTROL CLASSIFIED ACCESS.</Text></View></GlassPanel>
      ...
      <SectionTitle label="REFERRAL NETWORK" action={`${phase2.data?.referrals?.qualified||0} QUALIFIED`}/><GlassPanel glow><Text style={styles.commandTitle}>{phase2.data?.referrals?.code||'ALLOCATING CHANNEL'}</Text><Text style={styles.commandMeta}>INVITER +350 GC · NEW PILOT +200 GC · REWARD AFTER SUCCESSFUL ONBOARDING</Text><GlassButton label="COPY REFERRAL LINK" icon={Send} onPress={()=>void copyReferral()} active/></GlassPanel><View style={styles.referralMilestoneRail}>{(phase2.data?.progress?.referralMilestones||[]).map((milestone:any)=><View key={milestone.count} style={[styles.referralMilestone,milestone.achieved&&styles.gameBadgeEarned]}><Text style={styles.featuredBadgeSlot}>{milestone.count}</Text><Text style={styles.commandTitle}>{milestone.reward}</Text><Text style={styles.commandMeta}>{milestone.achieved?'SECURED':'LOCKED'}</Text></View>)}</View>
      <SectionTitle label="DRIVER MILESTONES" action={`${phase2.data?.progress?.milestones?.length||0} RECORDED`}/><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.shopCategoryRail}>{(phase2.data?.progress?.milestones||[]).map((milestone:any)=><View key={milestone.milestone_key} style={styles.tradeInstance}><BadgeCheck size={18} color={accent}/><Text style={styles.commandTitle}>{String(milestone.milestone_key).replaceAll('-',' ').toUpperCase()}</Text><Text style={styles.commandMeta}>{new Date(milestone.earned_at).toLocaleDateString()}</Text></View>)}</ScrollView>
      {(phase2.data?.progress?.vehicleLegacy||[]).length?<><SectionTitle label="VEHICLE LEGACY" action={`${phase2.data.progress.vehicleLegacy.length} BUILDS`}/>{phase2.data.progress.vehicleLegacy.map((legacy:any)=><View key={legacy.vehicle_id} style={styles.worldListRow}><CarFront size={20} color={accent}/><View style={styles.commandCopy}><Text style={styles.commandTitle}>{legacy.nickname||`${legacy.year} ${legacy.make} ${legacy.model}`}</Text><Text style={styles.commandMeta}>{Number(legacy.apex_miles||0).toFixed(1)} APEX MI · {legacy.meets} MEETS · {legacy.bounty_escapes} ESCAPES</Text></View><Text style={styles.dropValue}>{legacy.title||'BUILD'}</Text></View>)}</>:null}
      <SectionTitle label="UNDERGROUND BROADCAST" action="REAL EVENTS"/>{(phase2.data?.progress?.broadcasts||[]).slice(0,5).map((entry:any)=><View key={entry.id} style={styles.broadcastRow}><Radio size={14} color={accent}/><Text style={styles.broadcastText}>{entry.text}</Text><Text style={styles.commandMeta}>{new Date(entry.occurred_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</Text></View>)}
...
function MemberInviteScreen(){
  const [codes,setCodes]=useState<InviteCodeRow[]>([]);const [redemptions,setRedemptions]=useState<InviteRedemption[]>([]);const [label,setLabel]=useState('PILOT INVITE');const [limit,setLimit]=useState(3);const [burn,setBurn]=useState(false);const [busy,setBusy]=useState(false);const [error,setError]=useState('');
  const load=async()=>{try{const data=await cloudflareApi.request<{codes:InviteCodeRow[];redemptions:InviteRedemption[]}>('/api/invites');setCodes(data.codes);setRedemptions(data.redemptions||[]);setError('');}catch(reason){setError(reason instanceof Error?reason.message:'Invite network failed.');}};useEffect(()=>{void load();},[]);
...
function AccessPortal({onUnlock,onSignIn}:{onUnlock:(code:string)=>void;onSignIn:()=>void}){
  const [code,setCode]=useState('');const [status,setStatus]=useState('SECURE CHANNEL READY');const [unlocking,setUnlocking]=useState(false);const [denied,setDenied]=useState(false);const reveal=useRef(new Animated.Value(0)).current;const shake=useRef(new Animated.Value(0)).current;const lockPulse=useRef(new Animated.Value(0)).current;const inputRef=useRef<TextInput>(null);
  ...
  const verify=async()=>{if(!/^\d{6}$/.test(code)){reject('Enter all six digits');return;}setStatus('VERIFYING CREDENTIAL');try{await cloudflareApi.request('/api/invite/verify',{method:'POST',body:JSON.stringify({code})});playInterfaceSound('unlock');hapticResult('success');setUnlocking(true);setTimeout(()=>onUnlock(code),720);}catch(error){reject(error instanceof Error?error.message:'Code unavailable');}};
  const enterKey=(key:string)=>{hapticTick();if(key==='CLEAR'){setCode('');setDenied(false);setStatus('SECURE CHANNEL READY');return;}if(key==='UNLOCK'){void verify();return;}setCode(current=>(current+key).replace(/\D/g,'').slice(0,6));setDenied(false);setStatus('SECURE CHANNEL READY');};
  if(unlocking)return <CredentialTransition/>;
  const pulseScale=lockPulse.interpolate({inputRange:[0,1],outputRange:[1,1.025]});
  const pulseOpacity=lockPulse.interpolate({inputRange:[0,1],outputRange:[.18,.38]});
  const shimmerTranslate=lockPulse.interpolate({inputRange:[0,1],outputRange:[-12,12]});
  const keys=['1','2','3','4','5','6','7','8','9','CLEAR','0','UNLOCK'];
  return <View style={[styles.accessPortal,denied&&styles.accessPortalDenied]}>
    <View style={styles.lockReferenceFrame}>
    <ImageBackground source={require('./assets/apex-lock-reference.png')} resizeMode="cover" style={styles.lockReferenceImage}>
      <LinearGradient colors={denied?['rgba(90,0,8,.30)','rgba(0,0,0,.06)','rgba(0,0,0,.22)']:['rgba(0,0,0,.10)','rgba(0,0,0,.02)','rgba(0,0,0,.16)']} style={StyleSheet.absoluteFill}/>
      <Animated.View pointerEvents="none" style={[styles.photoLockGlow,{opacity:pulseOpacity,transform:[{scale:pulseScale}]}]}/>
      <Animated.View pointerEvents="none" style={[styles.photoScanLine,{transform:[{translateY:shimmerTranslate}]}]}/>
      {denied?<View pointerEvents="none" style={styles.deniedGlitch}><View style={[styles.glitchLine,{top:'33%'}]}/><View style={[styles.glitchLine,{top:'58%'}]}/><Text style={styles.deniedStamp}>ACCESS DENIED</Text></View>:null}
      <Animated.View style={[styles.photoLockControls,{opacity:reveal,transform:[{translateX:shake}]}]}>
        <Pressable accessibilityRole="button" accessibilityLabel="Enter six digit access code" onPress={()=>inputRef.current?.focus()} style={styles.photoCodeSlots}>
          {Array.from({length:6},(_,index)=><Text key={index} style={[styles.photoCodeDigit,denied&&styles.photoLockCodeDenied]}>{code[index]||''}</Text>)}
        </Pressable>
        <TextInput ref={inputRef} value={code} onChangeText={value=>{setCode(value.replace(/\D/g,'').slice(0,6));setDenied(false);setStatus('SECURE CHANNEL READY');hapticTick();}} keyboardType="number-pad" inputMode="numeric" maxLength={6} caretHidden style={styles.photoLockNativeInput} onSubmitEditing={()=>void verify()} autoFocus={Platform.OS==='web'}/>
        <View style={styles.photoStatusRow}><View style={[styles.accessStatusDot,code.length===6&&!denied&&styles.accessStatusDotReady,denied&&styles.accessStatusDotDenied]}/><Text numberOfLines={1} style={[styles.photoStatusText,denied&&styles.accessStatusTextDenied]}>{status}</Text></View>
        <View style={styles.photoKeypad}>{keys.map(key=><Pressable key={key} accessibilityLabel={key==='UNLOCK'?'Unlock network':key==='CLEAR'?'Clear access code':`Enter ${key}`} onPress={()=>enterKey(key)} style={({pressed})=>[styles.photoKeyHit,pressed&&styles.photoKeyHitPressed,key==='UNLOCK'&&code.length===6&&styles.photoKeyHitReady]}/>)}</View>
        <Pressable onPress={onSignIn} style={({pressed})=>[styles.photoExistingHit,pressed&&{opacity:.7}]}><Text style={styles.photoExistingText}>ALREADY CLEARED? SIGN IN</Text></Pressable>
      </Animated.View>
    </ImageBackground>
    </View>
  </View>;
...
function AuthPanel({ onClose, onOpen, initialMode='signin', inviteCode }: { onClose: () => void;onOpen?: (tab:TabKey)=>void;initialMode?:'signin'|'signup';inviteCode?:string|null }) {
  ...
  return <View style={styles.authOverlay}><Pressable onPress={onClose} style={StyleSheet.absoluteFill}/><GlassPanel style={styles.authPanel} glow><View style={styles.authBrandHeader}><Image source={require('../../assets/apex-brand-mark.png')} resizeMode="contain" style={styles.authBrandMark}/><View style={styles.commandCopy}><Text style={styles.authBrandName}>APEX UGR</Text><Text style={styles.authBrandMeta}>SECURE PILOT IDENTITY</Text></View><Pressable onPress={onClose} style={styles.closeButton}><X size={17} color={paper}/></Pressable></View><View style={styles.authHeadingBlock}><Text style={styles.authTitle}>{mode==='signin'?'Welcome back.':'Create your pilot.'}</Text><Text style={styles.authLead}>{mode==='signin'?'Sign in to reopen your private channel.':'Your invite reserves access to the Apex network.'}</Text></View>{hasCloudflareBackend?<>{mode==='signup'?<View style={styles.freeAccountBanner}><ShieldCheck size={17} color={accent}/><View><Text style={styles.freeAccountTitle}>{inviteCode?'INVITE VERIFIED':'DEVELOPER REGISTRATION'}</Text><Text style={styles.freeAccountMeta}>{inviteCode?`CODE ${inviteCode} // SLOT RESERVED`:'OWNER EMAIL REQUIRED'}</Text></View></View>:null}<ScrambleReadout value={email} label="IDENTITY SIGNAL"/><Text style={styles.authFieldLabel}>EMAIL</Text><TextInput value={email} onChangeText={value=>{setEmail(value);playInterfaceSound('key');}} autoCapitalize="none" autoComplete="email" keyboardType="email-address" placeholder="driver@example.com" placeholderTextColor={muted} style={styles.authInput}/><ScrambleReadout value={password} masked label="PASSWORD SCRAMBLE"/><Text style={styles.authFieldLabel}>PASSWORD</Text><TextInput value={password} onChangeText={value=>{setPassword(value);playInterfaceSound('key');}} secureTextEntry autoComplete={mode==='signin'?'current-password':'new-password'} placeholder="8 or more characters" placeholderTextColor={muted} style={styles.authInput} onSubmitEditing={submit}/><GlassButton label={mode==='signin'?'SIGN IN':'CREATE PILOT'} icon={LockKeyhole} onPress={submit} active/><Pressable onPress={()=>{setMode(value=>value==='signin'?'signup':'signin');setStatus('');}} style={styles.authSwitchButton}><Text style={styles.authSwitch}>{mode==='signin'?'HAVE AN INVITE? CREATE ACCOUNT':'ALREADY A MEMBER? SIGN IN'}</Text><ChevronRight size={14} color={accent}/></Pressable></>:<View style={styles.emptyNotification}><Radio size={26} color={accent}/><Text style={styles.emptyTitle}>BACKEND CONNECTION REQUIRED</Text><Text style={styles.emptyCopy}>The app will not invent an account or local network data.</Text></View>}{status?<Text style={styles.networkError}>{status}</Text>:null}</GlassPanel></View>;
}
...
const styles = StyleSheet.create({
  gameLobby: { flex: 1, minHeight: '100%', backgroundColor: '#010201', overflow: 'hidden', paddingHorizontal: 16, paddingTop: Platform.OS === 'web' ? 22 : 38, paddingBottom: Platform.OS === 'web' ? 20 : 34 },
  ...
  cinematicStory: { position: 'absolute', left: 22, right: 22, bottom: Platform.OS === 'web' ? 74 : 112, alignItems: 'center' },
  cinematicReadout: { alignItems: 'center' },
  skipIntroButton: { position: 'absolute', left: 22, right: 22, bottom: Platform.OS === 'web' ? 20 : 38, minHeight: 42, borderRadius: 9, borderWidth: 1, borderColor: 'rgba(167,229,154,.52)', backgroundColor: 'rgba(3,10,5,.86)', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, zIndex: 20 },
  skipIntroText: { color: accent, fontSize: 9, fontWeight: '900', letterSpacing: 1.35 },
  ...
  apexLogoImage: { width: 38, height: 38 },
  ...
  dailyChest: { minHeight: 78, marginTop: 10, padding: 12, borderRadius: 11, borderWidth: 1, borderColor: border, backgroundColor: 'rgba(8,11,9,.88)', flexDirection: 'row', alignItems: 'center', gap: 11 },
  dailyChestReady: { borderColor: 'rgba(167,229,154,.48)', backgroundColor: 'rgba(15,29,17,.82)' },
  dailyChestOpened: { borderColor: 'rgba(167,229,154,.78)', backgroundColor: 'rgba(29,55,30,.76)' },
  dailyChestCore: { width: 48, height: 48, borderRadius: 14, borderWidth: 1, borderColor: 'rgba(255,255,255,.22)', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,255,255,.04)' },
  dailyChestCoreActive: { borderColor: accent, shadowColor: accent, shadowOpacity: .42, shadowRadius: 14, transform: [{ rotate: '12deg' }] },
  dailyChestCoreOpened: { transform: [{ rotate: '0deg' }], backgroundColor: 'rgba(167,229,154,.14)' },
  dailyChestProgress: { flexDirection: 'row', gap: 5, marginTop: 8 },
  dailyChestProgressDot: { width: 18, height: 2, backgroundColor: 'rgba(255,255,255,.16)' },
  dailyChestProgressDotActive: { backgroundColor: accent },
  ...
  locateButtonActive: { backgroundColor: accent, borderColor: 'rgba(255,255,255,.82)' },
  locateText: { color: paper, fontSize: 8, fontWeight: '900', letterSpacing: 0.7 },
  locateTextActive: { color: '#041006' },
  ...
  cachePrompt: { color: paper, fontSize: 10, fontWeight: '900', textAlign: 'center', marginTop: 10 },
  ...
  garageSwitcher: { flexDirection: 'row', gap: 7, marginBottom: 10, alignItems: 'center' },
  garageVehicleRail: { gap: 7, paddingRight: 8 },
  carChip: { minWidth: 144, maxWidth: 210, minHeight: 42, paddingHorizontal: 10, borderWidth: 1, borderColor: border, borderRadius: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, backgroundColor: 'rgba(255,255,255,.04)' },
  ...
  carChipOrdinal: { color: muted, fontSize: 7, fontWeight: '900' },
  ...
  garageCarousel: { marginHorizontal: -16 },
  garageCarouselPage: { width: screenWidth - 32, marginHorizontal: 16 },
  garageDots: { flexDirection: 'row', alignSelf: 'center', gap: 6, marginTop: 9 },
  garageDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: 'rgba(255,255,255,.22)' },
  garageDotActive: { width: 18, backgroundColor: accent },
  buildTotals: { marginTop: 9, minHeight: 45, paddingHorizontal: 12, borderRadius: 9, borderWidth: 1, borderColor: 'rgba(167,229,154,.24)', backgroundColor: 'rgba(167,229,154,.06)', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 9 },
  buildTotalValue: { color: accent, fontSize: 8, fontWeight: '900', textAlign: 'right', flexShrink: 1 },
  ...
  moduleCard: { width: '47%', minHeight: 142, borderRadius: 12, borderWidth: 1, borderColor: border, backgroundColor: 'rgba(5,8,6,.78)', padding: 13, position: 'relative' },
  ...
  feedListViewport: { flex: 1, minHeight: 0 },
  ...
  feedVideoRetry: { position: 'absolute', alignSelf: 'center', top: '42%', minHeight: 42, paddingHorizontal: 14, borderRadius: 9, borderWidth: 1, borderColor: 'rgba(167,229,154,.46)', backgroundColor: 'rgba(2,7,4,.88)', flexDirection: 'row', alignItems: 'center', gap: 8 },
  feedVideoRetryText: { color: paper, fontSize: 9, fontWeight: '900', letterSpacing: 1 },
  ...
  feedActionRail: { position: 'absolute', right: 10, bottom: 88, width: 50, alignItems: 'center', gap: 9 },
  ...
  feedCommentPanel: { position: 'absolute', left: 12, right: 64, bottom: 82, zIndex: 24, maxHeight: 220, padding: 9, gap: 8, borderRadius: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,.24)', backgroundColor: 'rgba(3,6,4,.97)' },
  feedCommentList: { maxHeight: 142 },
  feedCommentRow: { paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,.08)' },
  feedCommentAlias: { color: accent, fontSize: 9, fontWeight: '900' },
  feedCommentBody: { color: paper, fontSize: 11, lineHeight: 16, marginTop: 2 },
  feedCommentEmpty: { color: muted, fontSize: 9, fontWeight: '800', textAlign: 'center', paddingVertical: 12 },
  feedCommentComposer: { minHeight: 46, paddingHorizontal: 8, flexDirection: 'row', alignItems: 'center', gap: 8 },
  ...
  authOverlay: { ...StyleSheet.absoluteFillObject, zIndex: 130, backgroundColor: 'rgba(0,0,0,.84)', alignItems: 'center', justifyContent: 'center', padding: 14 },
  authPanel: { width: '100%', maxWidth: 420, borderRadius: 8, borderColor: 'rgba(255,255,255,.20)' },
  authBrandHeader: { minHeight: 54, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,.10)', paddingBottom: 12, marginBottom: 18 },
  authBrandMark: { width: 40, height: 40, marginRight: 9 },
  authBrandName: { color: paper, fontSize: 13, fontWeight: '900', letterSpacing: 1.8 },
  authBrandMeta: { color: muted, fontSize: 6, fontWeight: '800', marginTop: 4, letterSpacing: .8 },
  authHeadingBlock: { marginBottom: 14 },
  authTitle: { color: paper, fontSize: 24, fontWeight: '800', letterSpacing: 0 },
  authLead: { color: '#AEB6B0', fontSize: 9, lineHeight: 14, marginTop: 6 },
  authFieldLabel: { color: '#C9D0CB', fontSize: 6, fontWeight: '900', letterSpacing: 1, marginTop: 9 },
  authInput: { minHeight: 50, color: paper, borderRadius: 7, borderWidth: 1, borderColor: 'rgba(255,255,255,.18)', backgroundColor: 'rgba(255,255,255,.04)', paddingHorizontal: 13, fontSize: 11, marginTop: 5, marginBottom: 5 },
  authSwitchButton: { minHeight: 42, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, marginTop: 5 },
  authSwitch: { color: accent, fontSize: 7, fontWeight: '900', textAlign: 'center', letterSpacing: .6 },
  ...
  accessGrid: { ...StyleSheet.absoluteFillObject, opacity: .28 },
  accessGridLine: { position: 'absolute', left: 0, right: 0, height: 1, backgroundColor: 'rgba(255,255,255,.025)' },
  accessStage: { flexGrow: 1, width: '100%', maxWidth: 480, minHeight: 720, alignSelf: 'center', paddingHorizontal: 18, paddingTop: Platform.OS==='web'?24:12, paddingBottom: 22, justifyContent: 'space-between' },
  accessBrandRow: { minHeight: 54, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,.10)', paddingBottom: 12 },
  accessBrandName: { color: paper, fontSize: 15, fontWeight: '900', letterSpacing: 2.2 },
  accessBrandMeta: { color: muted, fontSize: 6, fontWeight: '800', marginTop: 4, letterSpacing: .9 },
  accessNode: { color: accent, fontSize: 6, fontWeight: '900', letterSpacing: 1.1 },
  accessGate: { width: '100%', maxWidth: 400, alignSelf: 'center', alignItems: 'center', paddingTop: 24 },
  accessSeal: { width: 84, height: 84, borderRadius: 42, borderWidth: 1, borderColor: 'rgba(255,255,255,.24)', backgroundColor: 'rgba(255,255,255,.035)', alignItems: 'center', justifyContent: 'center', marginBottom: 18, shadowColor: accent, shadowOpacity: .16, shadowRadius: 22 },
  accessSealInner: { width: 62, height: 62, borderRadius: 31, borderWidth: 1, borderColor: 'rgba(110,242,195,.26)', alignItems: 'center', justifyContent: 'center' },
  accessSealSignal: { position: 'absolute', right: 7, top: 14, width: 7, height: 7, borderRadius: 4, backgroundColor: accent, borderWidth: 1, borderColor: '#050706' },
  accessKicker: { color: accent, fontSize: 7, fontWeight: '900', letterSpacing: 1.8 },
  accessTitle: { color: paper, fontSize: 28, fontWeight: '800', marginTop: 8, letterSpacing: 0 },
  accessDescription: { maxWidth: 320, color: '#AEB6B0', fontSize: 9, lineHeight: 15, textAlign: 'center', marginTop: 7 },
  accessCodeRow: { width: '100%', flexDirection: 'row', gap: 7, marginTop: 22 },
  accessCodeSlot: { flex: 1, aspectRatio: .82, maxHeight: 60, borderRadius: 7, borderWidth: 1, borderColor: 'rgba(255,255,255,.16)', backgroundColor: 'rgba(255,255,255,.035)', alignItems: 'center', justifyContent: 'center' },
  accessCodeSlotFilled: { borderColor: 'rgba(110,242,195,.48)', backgroundColor: 'rgba(110,242,195,.075)' },
  accessCodeSlotDenied: { borderColor: 'rgba(255,86,98,.68)', backgroundColor: 'rgba(92,0,8,.18)' },
  accessCodeSlotText: { color: paper, fontSize: 22, fontWeight: '700', letterSpacing: 0 },
  accessCodeSlotTextDenied: { color: '#FFB1B7' },
  accessNativeInput: { position: 'absolute', width: 1, height: 1, opacity: .01, left: '50%', top: 220 },
  accessStatusRow: { width: '100%', minHeight: 36, flexDirection: 'row', alignItems: 'center', gap: 7, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,.08)' },
  accessStatusDot: { width: 5, height: 5, borderRadius: 3, backgroundColor: '#6F7771' },
  accessStatusDotReady: { backgroundColor: accent, shadowColor: accent, shadowOpacity: .8, shadowRadius: 7 },
  accessStatusDotDenied: { backgroundColor: '#FF5662', shadowColor: '#FF5662', shadowOpacity: .8, shadowRadius: 7 },
  accessStatusText: { flex: 1, color: '#9EA7A1', fontSize: 6, fontWeight: '900', letterSpacing: .9 },
  accessStatusTextDenied: { color: '#FF8991' },
  accessDigitCount: { color: muted, fontSize: 7, fontWeight: '800' },
  accessKeypad: { width: '100%', flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', rowGap: 8, marginTop: 14 },
  accessKey: { width: '31.7%', height: 54, borderRadius: 8, borderWidth: 1, borderColor: 'rgba(255,255,255,.15)', backgroundColor: 'rgba(255,255,255,.045)', alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 5, overflow: 'hidden' },
  accessKeyPressed: { borderColor: 'rgba(110,242,195,.58)', backgroundColor: 'rgba(110,242,195,.12)', transform: [{scale:.97}] },
  accessKeyReady: { borderColor: accent, backgroundColor: accent },
  accessKeyNumber: { color: paper, fontSize: 20, fontWeight: '500', letterSpacing: 0 },
  accessKeySignal: { position: 'absolute', bottom: 3, color: 'rgba(110,242,195,.5)', fontSize: 10 },
  accessKeyAction: { color: paper, fontSize: 6, fontWeight: '900', letterSpacing: .8 },
  accessKeyActionReady: { color: '#061008' },
  accessExisting: { minHeight: 42, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, marginTop: 8 },
  accessExistingText: { color: '#C9D0CB', fontSize: 7, fontWeight: '900', letterSpacing: .8 },
  accessFooter: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingTop: 18 },
  accessFooterLine: { flex: 1, height: 1, backgroundColor: 'rgba(255,255,255,.10)' },
  accessFooterText: { color: '#68706A', fontSize: 5, fontWeight: '900', letterSpacing: .8 },
  lockReferenceFrame: { flex: 1, width: '100%', maxWidth: 520, alignSelf: 'center', backgroundColor: '#010101', overflow: 'hidden' },
  lockReferenceImage: { flex: 1, width: '100%', height: '100%', opacity: 1 },
  ...
  photoLockGlow: { position: 'absolute', width: '64%', aspectRatio: 1, left: '18%', top: '25.5%', borderRadius: 999, backgroundColor: 'rgba(151,255,167,.22)', shadowColor: '#EFFFF0', shadowOpacity: .96, shadowRadius: 48 },
  photoScanLine: { position: 'absolute', left: '22%', right: '22%', top: '37%', height: 1, backgroundColor: 'rgba(238,255,242,.82)', shadowColor: '#EFFFF0', shadowOpacity: .95, shadowRadius: 12 },
  photoLockControls: { ...StyleSheet.absoluteFillObject, width: '100%', height: '100%' },
  photoCodeSlots: { position: 'absolute', top: '56.25%', left: '20.5%', right: '20.5%', height: '5.1%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  photoCodeDigit: { width: '14%', color: paper, fontSize: 19, fontWeight: '900', textAlign: 'center', textShadowColor: '#B9FFF2', textShadowRadius: 5 },
  photoLockNativeInput: { position: 'absolute', top: '55.2%', left: '17%', right: '17%', height: '6.7%', opacity: 0.01, color: 'transparent' },
  photoLockCodeDenied: { color: '#FFB1B8', textShadowColor: '#FF3344', textShadowRadius: 12 },
  photoStatusRow: { position: 'absolute', top: '61.4%', left: '17%', right: '17%', minHeight: 25, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, backgroundColor: 'rgba(0,0,0,.12)' },
  photoStatusText: { color: '#DFFFE8', fontSize: 7, fontWeight: '900', letterSpacing: 1.1, textAlign: 'center', textShadowColor: '#8DFFAD', textShadowRadius: 5 },
  photoKeypad: { position: 'absolute', top: '64.3%', left: '18.7%', right: '18.7%', height: '22.1%', flexDirection: 'row', flexWrap: 'wrap' },
  photoKeyHit: { width: '33.333%', height: '25%' },
  photoKeyHitPressed: { backgroundColor: 'rgba(103,255,224,.08)' },
  photoKeyHitReady: { backgroundColor: 'rgba(167,229,154,.09)' },
  photoExistingHit: { position: 'absolute', bottom: '2.2%', left: '10%', right: '10%', minHeight: 38, alignItems: 'center', justifyContent: 'center' },
  ...
  bountySignalCard: { borderColor: 'rgba(44,255,131,.48)', backgroundColor: 'rgba(44,255,131,.08)' },
  frequencyRail: { position: 'absolute', top: 66, left: 12, right: 12, zIndex: 12, flexDirection: 'row', gap: 6 },
  frequencyChip: { minHeight: 31, paddingHorizontal: 9, borderRadius: 7, borderWidth: 1, borderColor: 'rgba(255,255,255,.24)', backgroundColor: 'rgba(4,7,5,.84)', alignItems: 'center', justifyContent: 'center' },
  frequencyChipActive: { borderColor: 'rgba(44,255,131,.7)', backgroundColor: 'rgba(44,255,131,.13)' },
  frequencyChipLocked: { opacity: .45 },
  frequencyChipText: { color: paper, fontSize: 7, fontWeight: '900', letterSpacing: .5 },
  bountyMapSignal: { position: 'absolute', top: 106, left: 12, right: 12, zIndex: 11, minHeight: 56, paddingHorizontal: 11, borderRadius: 9, borderWidth: 1, borderColor: 'rgba(44,255,131,.45)', backgroundColor: 'rgba(3,8,4,.91)', flexDirection: 'row', alignItems: 'center', gap: 9 },
  mostWantedPanel: { borderColor: 'rgba(255,255,255,.58)', backgroundColor: 'rgba(255,255,255,.065)' },
  bountyWorldMetrics: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginVertical: 15 },
  bountyReward: { color: accent, fontSize: 21, fontWeight: '900' },
  bountyHeat: { color: paper, fontSize: 21, fontWeight: '900' },
  ghostCoinHero: { flexDirection: 'row', alignItems: 'center', gap: 13 },
  ghostCoinMetal: { width: 64, height: 64, borderRadius: 32, borderWidth: 2, borderColor: '#A5F3A0', backgroundColor: '#101612', alignItems: 'center', justifyContent: 'center', shadowColor: accent, shadowOpacity: .34, shadowRadius: 14, overflow: 'hidden' },
  ghostCoinCircuit: { position: 'absolute', width: 52, height: 52, borderRadius: 26, borderWidth: 1, borderStyle: 'dashed', borderColor: 'rgba(44,255,131,.7)' },
  ghostCoinMark: { color: paper, fontSize: 27, fontWeight: '900' },
  ghostCoinValue: { color: paper, fontSize: 24, fontWeight: '900' },
  rarityLabel: { fontSize: 7, fontWeight: '900', letterSpacing: .6, marginTop: 5 },
  tradeInstance: { width: 155, minHeight: 62, padding: 9, borderRadius: 8, borderWidth: 1, borderColor: border, backgroundColor: 'rgba(255,255,255,.035)' },
  broadcastRow: { minHeight: 48, paddingHorizontal: 10, borderBottomWidth: 1, borderBottomColor: border, flexDirection: 'row', alignItems: 'center', gap: 9 },
  broadcastText: { flex: 1, color: paper, fontSize: 8, fontWeight: '800', lineHeight: 12 },
  referralMilestoneRail: { flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 8 },
  referralMilestone: { width: '48%', minHeight: 82, borderWidth: 1, borderColor: border, backgroundColor: 'rgba(255,255,255,.025)', padding: 10, justifyContent: 'space-between' },
});
