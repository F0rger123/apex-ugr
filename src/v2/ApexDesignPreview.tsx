...
function RadarScreen({ onTab }: { onTab: (tab: TabKey) => void }) {
  ...
  return (
    <View style={styles.radarScreen}>
      ...
      <View style={styles.frequencyRail}>{[1,7,13].map(level=><Pressable key={level} onPress={()=>void setFrequency(level)} style={[styles.frequencyChip,frequency?.activeLevel===level&&styles.frequencyChipActive,level>(frequency?.unlockedLevel||1)&&styles.frequencyChipLocked]}><Text style={styles.frequencyChipText}>FREQ {String(level).padStart(2,'0')}</Text></Pressable>)}</View>
      {worldBounty?<Pressable onPress={()=>onTab('bounty')} style={styles.bountyMapSignal}><Crosshair size={16} color={accent}/><View style={styles.commandCopy}><Text style={styles.commandTitle}>{worldBounty.starLevel===5?'MOST WANTED':'BOUNTY NETWORK'} · {'★'.repeat(worldBounty.starLevel)}</Text><Text style={styles.commandMeta}>{worldBounty.blackout?'GHOST NETWORK BLACKOUT':`${worldBounty.rewardGc} GC · HEAT ${worldBounty.heat}% · ${countdownLabel(worldBounty.endsAt)}`}</Text></View><ChevronRight size={15} color={accent}/></Pressable>:null}
