...
type TabKey = 'command' | 'radar' | 'feed' | 'garage' | 'more' | 'race' | 'vault' | 'shop' | 'parts' | 'meets' | 'messages' | 'leaderboard' | 'access' | 'settings' | 'bounty' | 'world';
type IconType = any;
  ...
  return (
    <View style={styles.feedScreen}>
      <View style={styles.feedFloatingHeader}>
        ...
        <View style={styles.sheetActions}><GlassButton label="CREWS" icon={Users} compact onPress={()=>onTab('world')}/><GlassButton label="COMMS" icon={MessagesSquare} compact onPress={()=>onTab('messages')}/><GlassButton label="POST" icon={Plus} compact onPress={() => setComposerOpen(value => !value)} active /></View>
      </View>
...
function MoreScreen({ onTab }: { onTab: (tab: TabKey) => void }) {
  ...
  const modules: { tab: TabKey; label: string; meta: string; icon: IconType }[] = [
    { tab: 'world', label: 'UNDERGROUND WORLD', meta: 'Crews, territory, drops, contracts', icon: Map },
    { tab: 'bounty', label: 'BOUNTY RADAR', meta: 'Opt-in venue sessions and Most Wanted', icon: Crosshair },
