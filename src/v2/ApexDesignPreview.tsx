...
type TabKey = 'command' | 'radar' | 'feed' | 'garage' | 'more' | 'race' | 'vault' | 'shop' | 'meets' | 'messages' | 'leaderboard' | 'access' | 'settings' | 'bounty' | 'world';
type IconType = any;
...
const tabPaths:Record<TabKey,string>={
  ...
  messages:'/app/social/messages',leaderboard:'/app/competition/leaderboards',access:'/app/settings/access',settings:'/app/settings',bounty:'/app/radar/bounty',world:'/app/world',
};
...
function CommandScreen({ onTab, onProfile }: { onTab: (tab: TabKey) => void; onProfile: () => void }) {
  ...
  const [chest,setChest]=useState<{available:boolean;streakCount:number}>({available:false,streakCount:0});
  const [claimingChest,setClaimingChest]=useState(false);
  const loadChest=async()=>{if(!profile)return;try{const data=await cloudflareApi.request<{available:boolean;streakCount:number}>('/api/daily-chest/status');setChest(data);}catch{/* HUD stays usable when the reward service is unavailable. */}};
  ...
  return (
    <ScrollView contentContainerStyle={styles.screenContent} showsVerticalScrollIndicator={false}>
      ...
      <Pressable onPress={()=>void claimChest()} disabled={!chest.available||claimingChest} style={[styles.ghostProtocol,!chest.available&&styles.worldClaimed]}><Gift size={21} color={chest.available?accent:muted}/><View style={styles.commandCopy}><Text style={styles.commandTitle}>DAILY GHOST CHEST</Text><Text style={styles.commandMeta}>{chest.available?'SIGNAL READY · TAP TO DECRYPT':`CLAIMED TODAY · ${chest.streakCount} DAY STREAK`}</Text></View><Text style={styles.sectionAction}>{claimingChest?'OPENING':chest.available?'OPEN':'SECURED'}</Text></Pressable>
...
function BountyScreen({onTab}:{onTab:(tab:TabKey)=>void}){
  ...
  const begin=async()=>{if(!settings?.bountyModeEnabled){setStatus('ENABLE BOUNTY MODE FIRST');return;}if(!isDriving){await startDrive();if(!useLiveNetworkStore.getState().isDriving){setStatus('DRIVER MODE AND LOCATION ARE REQUIRED');return;}}setBusy(true);try{await cloudflareApi.request('/api/bounty/trigger',{method:'POST',body:JSON.stringify({mode:'venue',venueName,starLevel:1})});await load();playInterfaceSound('unlock');}catch(error){setStatus(error instanceof Error?error.message:'Could not activate Bounty.');}finally{setBusy(false);}};
        ...
        <View style={styles.tabBarShell}>
          <BlurView intensity={42} tint="dark" style={styles.tabBar}>
            {tabs.map(item => {
              ...
              const activeTab = tab === item.key || (item.key === 'command' && ['vault', 'leaderboard', 'access', 'settings', 'world', 'more'].includes(tab)) || (item.key === 'radar' && ['meets','bounty'].includes(tab)) || (item.key === 'feed' && ['messages'].includes(tab));
              return (
