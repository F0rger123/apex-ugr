...
function LegacySettingsScreen(){
  ...
  const save=async()=>{if(!settings||busy)return;setBusy(true);try{await Promise.all([cloudflareApi.request('/api/settings',{method:'PUT',body:JSON.stringify(settings)}),cloudflareApi.request('/api/bounty/settings',{method:'PUT',body:JSON.stringify({bounty_mode_enabled:bounty,agreed:bounty,notifications_enabled:true,show_public_photo:true,allow_most_wanted:true})}),AsyncStorage.setItem(LOCAL_SETTINGS_KEY,JSON.stringify({audio,haptics}))]);setInterfaceAudioEnabled(audio);setHapticsEnabled(haptics);const liveUnit=useLiveNetworkStore.getState().unit.toUpperCase();if((settings.unit_preference==='KMH'?'KPH':'MPH')!==liveUnit)useLiveNetworkStore.getState().toggleUnit();playInterfaceSound('unlock');setStatus('SETTINGS SECURED');}catch(error){setStatus(error instanceof Error?error.message:'Settings could not be saved.');}finally{setBusy(false);}};
...
function SettingsScreen(){
  ...
  const save=async()=>{if(!settings||busy)return;setBusy(true);try{await Promise.all([cloudflareApi.request('/api/settings',{method:'PUT',body:JSON.stringify(settings)}),cloudflareApi.request('/api/bounty/settings',{method:'PUT',body:JSON.stringify({bounty_mode_enabled:bounty,agreed:bounty,notifications_enabled:Boolean(settings.bounty_notifs_enabled),show_public_photo:Boolean(settings.profile_visibility),allow_most_wanted:true})}),AsyncStorage.setItem(LOCAL_SETTINGS_KEY,JSON.stringify({audio,haptics}))]);setInterfaceAudioEnabled(audio);setHapticsEnabled(haptics);const liveUnit=useLiveNetworkStore.getState().unit.toUpperCase();if((settings.unit_preference==='KMH'?'KPH':'MPH')!==liveUnit)useLiveNetworkStore.getState().toggleUnit();playInterfaceSound('unlock');setStatus('SETTINGS SECURED');}catch(error){setStatus(error instanceof Error?error.message:'Settings could not be saved.');}finally{setBusy(false);}};
...
function BountyScreen({onTab}:{onTab:(tab:TabKey)=>void}){
  ...
  const saveOptIn=async(enabled:boolean)=>{if(enabled&&!accepted){setStatus('ACCEPT THE SAFETY AGREEMENT FIRST');return;}setBusy(true);try{await cloudflareApi.request('/api/bounty/settings',{method:'PUT',body:JSON.stringify({bounty_mode_enabled:enabled,agreed:accepted,notifications_enabled:true,show_public_photo:true,allow_most_wanted:true})});await load();setStatus(enabled?'BOUNTY NETWORK ARMED':'BOUNTY NETWORK DISARMED');}catch(error){setStatus(error instanceof Error?error.message:'Could not update Bounty Mode.');}finally{setBusy(false);}};
