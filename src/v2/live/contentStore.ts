...
export type RaceContract={id:string;challengerId:string;challengerName:string;raceType:string;routeName:string;startsAt:string;wagerCredits:number;status:string;rescheduleCount:number;isChallenger:boolean;participants:Array<{userId:string;status:string;username:string;tier:string;reputation:number}>};
...
export const useContentStore=create<ContentState>((set,get)=>({
  ...
  loadRaces:async()=>{
    if(!get().userId)return;
    try{const data=await cloudflareApi.request<{races:any[]}>('/api/races');set({races:data.races.map(row=>({id:row.id,challengerId:row.challenger_id,challengerName:row.challenger_name,raceType:row.race_type,routeName:row.route_name,startsAt:row.starts_at,wagerCredits:Number(row.wager_credits||0),status:row.status,rescheduleCount:Number(row.reschedule_count||0),isChallenger:Boolean(row.is_challenger),participants:(row.participants||[]).map((pilot:any)=>({userId:pilot.user_id,status:pilot.status,username:pilot.username,tier:pilot.tier,reputation:Number(pilot.reputation||1000)}))}))});}catch(error){set({error:error instanceof Error?error.message:'Race inbox failed'});}
  },
