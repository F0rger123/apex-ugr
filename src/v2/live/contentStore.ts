...
export type ActiveVehicle = { id:string; nickname:string; year:number; make:string; model:string; trim:string|null; engine:string; drivetrain:string; horsepower:number; color:string; vehicleType:'CAR'|'MOTORCYCLE'; displacementCc:number|null; photoUrl:string|null;digitalTwinUrl:string|null;digitalTwinStatus:string };
...
export const useContentStore=create<ContentState>((set,get)=>({
  ...
  loadVehicles:async()=>{
    if(!get().userId)return;
    try{const data=await cloudflareApi.request<{vehicles:any[]}>('/api/vehicles');const vehicles:ActiveVehicle[]=data.vehicles.map(row=>({id:row.id,nickname:row.nickname,year:Number(row.year),make:row.make,model:row.model,trim:row.trim||null,engine:row.engine||'',drivetrain:row.drivetrain||'',horsepower:Number(row.horsepower||0),color:row.color||'',vehicleType:(row.vehicle_type==='MOTORCYCLE'?'MOTORCYCLE':'CAR') as 'CAR'|'MOTORCYCLE',displacementCc:row.displacement_cc===null?null:Number(row.displacement_cc||0),photoUrl:row.photo_url||null,digitalTwinUrl:row.digital_twin_url||null,digitalTwinStatus:row.digital_twin_status||'not_started'}));const activeRow=data.vehicles.find(row=>Boolean(row.is_active));set({vehicles,activeVehicleId:activeRow?.id||vehicles[0]?.id||null});}catch(error){set({error:error instanceof Error?error.message:'Garage failed'});}
  },
