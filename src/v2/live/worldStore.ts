...
export type RoadReport={id:string;type:'fixed_camera'|'hazard'|'closure'|'dangerous_road'|'speed_trap'|'police_sighting';note:string;latitude:number;longitude:number;heading_degrees:number|null;confirmations:number;username:string;created_at:string};
...
interface WorldState{discoveries:Discovery[];crewDiscoveries:Discovery[];discoveryChain:{current:number;best:number};territories:Territory[];drops:DeadDrop[];reports:RoadReport[];crews:Crew[];crewRequests:CrewRequest[];seasons:Season[];journeys:SeasonJourney[];safeHouses:SafeHouse[];badges:GameBadge[];contracts:Contract[];rewards:MapReward[];ghostReplays:GhostReplay[];heat:number;loading:boolean;error:string|null;
  refresh:()=>Promise<void>;report:(type:RoadReport['type'],note:string,latitude:number,longitude:number,headingDegrees?:number|null)=>Promise<boolean>;confirmReport:(id:string)=>Promise<void>;createCrew:(name:string,tag:string)=>Promise<boolean>;joinCrew:(id:string)=>Promise<void>;approveMember:(crewId:string,userId:string)=>Promise<void>;createTerritory:(crewId:string,data:{name:string;latitude:number;longitude:number;radiusM:number;requiredCells:number})=>Promise<boolean>;joinSeason:(id:string)=>Promise<void>;joinJourney:(id:string)=>Promise<void>;checkJourney:(id:string,location:{latitude:number;longitude:number;accuracy:number|null})=>Promise<{complete:boolean;rewardCredits:number}|null>;createSafeHouse:(data:{name:string;latitude:number;longitude:number;vehicleId?:string|null})=>Promise<boolean>;deleteSafeHouse:(id:string)=>Promise<void>;acceptContract:(id:string)=>Promise<void>;
}
