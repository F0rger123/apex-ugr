...
export const useLiveNetworkStore=create<LiveNetworkState>((set,get)=>({
  ...
  initialize:async()=>{
    ...
    const [session,cached,cachedOrigin,cachedRoute]=await Promise.all([cloudflareApi.session(),AsyncStorage.getItem(LOCATION_KEY),AsyncStorage.getItem(REVEAL_ORIGIN_KEY),AsyncStorage.getItem(ACTIVE_ROUTE_KEY)]);
    let location:LiveCoordinate|null=null,revealOrigin:LiveCoordinate|null=null,route:LiveRoute|null=null;
    ...
    if(cachedRoute){try{const parsed=JSON.parse(cachedRoute) as LiveRoute;if(parsed.destination&&Array.isArray(parsed.coordinates)&&parsed.coordinates.length>1)route=parsed;}catch{await AsyncStorage.removeItem(ACTIVE_ROUTE_KEY);}}
    if(location||route)set({location,revealOrigin:revealOrigin||location,route});
  ...
  setRoute:async destination=>{
    ...
    try{const data=await cloudflareApi.request<{destination:{name:string;latitude:number;longitude:number};stops?:RouteStop[];distanceKm:number;durationMinutes:number;coordinates:LiveRoute['coordinates']}>('/api/routes',{method:'POST',body:JSON.stringify({origin,destination})});const route={destination:data.destination.name,destinationLatitude:data.destination.latitude,destinationLongitude:data.destination.longitude,distanceKm:data.distanceKm,durationMinutes:data.durationMinutes,coordinates:data.coordinates,stops:data.stops||[data.destination]};set({route,suggestions:[],error:null});await AsyncStorage.setItem(ACTIVE_ROUTE_KEY,JSON.stringify(route));return true;}catch(error){set({error:error instanceof Error?error.message:'Route unavailable. Try another destination.'});return false;}
  },
  setMultiStopRoute:async stops=>{const origin=get().location;if(!origin){set({error:'Lock GPS before building an itinerary.'});return false;}if(!stops.length){set({error:'Add at least one stop.'});return false;}try{const data=await cloudflareApi.request<{destination:RouteStop;stops:RouteStop[];distanceKm:number;durationMinutes:number;coordinates:LiveRoute['coordinates']}>('/api/routes',{method:'POST',body:JSON.stringify({origin,stops})});const route={destination:data.destination.name,destinationLatitude:data.destination.latitude,destinationLongitude:data.destination.longitude,distanceKm:data.distanceKm,durationMinutes:data.durationMinutes,coordinates:data.coordinates,stops:data.stops};set({route,suggestions:[],error:null});await AsyncStorage.setItem(ACTIVE_ROUTE_KEY,JSON.stringify(route));return true;}catch(error){set({error:error instanceof Error?error.message:'Route unavailable. Check the stop order and retry.'});return false;}},
  setRouteToPoint:async(name,latitude,longitude)=>{let origin=get().location;if(!origin){try{await get().lockLocation();origin=get().location;}catch{}}if(!origin){origin={latitude:34.0522,longitude:-118.2437,accuracy:10,altitude:null,heading:0,speedKph:0,timestamp:Date.now()};}try{const data=await cloudflareApi.request<{destination:{name:string;latitude:number;longitude:number};stops?:RouteStop[];distanceKm:number;durationMinutes:number;coordinates:LiveRoute['coordinates']}>('/api/routes',{method:'POST',body:JSON.stringify({origin,destination:name,target:{latitude,longitude}})});const route={destination:data.destination.name,destinationLatitude:data.destination.latitude,destinationLongitude:data.destination.longitude,distanceKm:data.distanceKm,durationMinutes:data.durationMinutes,coordinates:data.coordinates,stops:data.stops||[data.destination]};set({route,error:null});await AsyncStorage.setItem(ACTIVE_ROUTE_KEY,JSON.stringify(route));return true;}catch(error){set({error:error instanceof Error?error.message:'Route unavailable. Try another destination.'});return false;}},
  restoreRoute:route=>{const active={destination:route.destination_name,destinationLatitude:Number(route.destination_latitude),destinationLongitude:Number(route.destination_longitude),distanceKm:Number(route.distance_km),durationMinutes:Number(route.duration_minutes),coordinates:route.coordinates,stops:route.stops||[]};set({route:active,suggestions:[],error:null});void AsyncStorage.setItem(ACTIVE_ROUTE_KEY,JSON.stringify(active));},
  ...
  clearRoute:()=>{set({route:null,suggestions:[]});void AsyncStorage.removeItem(ACTIVE_ROUTE_KEY);},
