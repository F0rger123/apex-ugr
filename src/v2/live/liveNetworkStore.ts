...
export const useLiveNetworkStore=create<LiveNetworkState>((set,get)=>({
  ...
  setRoute:async destination=>{
    // setRouteToPoint (routing to a search result) already tolerates a
    // missing GPS fix by acquiring one first; this one required it to
    // already exist and failed outright otherwise -- a common way to hit
    // "navigation is bad" if the initial lock (RadarScreen's own mount
    // effect) hadn't resolved yet by the time someone typed a destination.
    let origin=get().location;if(!isValidCoordinate(origin)){try{await get().lockLocation();origin=get().location;}catch{}}
    if(!isValidCoordinate(origin)){set({error:'Lock GPS before setting a route.'});return false;}
  ...
  renamePlace:async(id,label)=>{const nickname=label.trim();if(!nickname)return;try{await cloudflareApi.request(`/api/places/${id}`,{method:'PUT',body:JSON.stringify({label:nickname})});await get().loadNavigation();}catch(error){set({error:error instanceof Error?error.message:'Could not rename this place.'});}},
  deletePlace:async id=>{try{await cloudflareApi.request(`/api/places/${id}`,{method:'DELETE'});await get().loadNavigation();}catch(error){set({error:error instanceof Error?error.message:'Could not delete this place.'});}},
  deleteSavedRoute:async id=>{try{await cloudflareApi.request(`/api/routes/${id}`,{method:'DELETE'});await get().loadNavigation();}catch(error){set({error:error instanceof Error?error.message:'Could not delete this route.'});}},
  renameSavedRoute:async(id,name)=>{const label=name.trim();if(!label)return;try{await cloudflareApi.request(`/api/routes/${id}`,{method:'PUT',body:JSON.stringify({name:label})});await get().loadNavigation();}catch(error){set({error:error instanceof Error?error.message:'Could not rename this route.'});}},
