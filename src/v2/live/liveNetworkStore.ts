...
import { normalizeSpeedUnit, SpeedUnit } from '../utils/units';

const LOCATION_KEY='apex.last-location';
const REVEAL_ORIGIN_KEY='apex.reveal-origin';
const UNIT_KEY='apex.speed-unit';
...
const REVEAL_ORIGIN_MAX_AGE_MS=24*60*60_000;
const LIVE_LOCATION_MAX_AGE_MS=30_000;
const MAX_USABLE_ACCURACY_M=85;
const MAX_PLAUSIBLE_SPEED_KPH=360;
...
function isFreshCoordinate(point:LiveCoordinate|null|undefined,maxAgeMs:number){return Boolean(point&&isValidCoordinate(point)&&Number.isFinite(point.timestamp)&&Date.now()-point.timestamp>=0&&Date.now()-point.timestamp<=maxAgeMs);}
function isUsableLiveCoordinate(point:LiveCoordinate|null|undefined){return Boolean(isFreshCoordinate(point,LIVE_LOCATION_MAX_AGE_MS)&&(point!.accuracy==null||point!.accuracy<=MAX_USABLE_ACCURACY_M));}
function shouldAcceptMovement(previous:LiveCoordinate|null|undefined,next:LiveCoordinate){if(!isUsableLiveCoordinate(next))return false;if(!previous||!isUsableLiveCoordinate(previous))return true;const elapsedHours=Math.max((next.timestamp-previous.timestamp)/3_600_000,1/3600);const impliedSpeed=segmentKm(previous,next)/elapsedHours;return impliedSpeed<=MAX_PLAUSIBLE_SPEED_KPH;}
...
export const useLiveNetworkStore=create<LiveNetworkState>((set,get)=>({
  ...
  initialize:async()=>{
    ...
    const [session,cached,cachedOrigin,cachedRoute,cachedNavigation,cachedUnit]=await Promise.all([cloudflareApi.session(),AsyncStorage.getItem(LOCATION_KEY),AsyncStorage.getItem(REVEAL_ORIGIN_KEY),AsyncStorage.getItem(ACTIVE_ROUTE_KEY),AsyncStorage.getItem(ACTIVE_NAVIGATION_KEY),AsyncStorage.getItem(UNIT_KEY)]);
    ...
    const unit=normalizeSpeedUnit(cachedUnit);
    if(cached){try{const parsed=JSON.parse(cached) as LiveCoordinate;if(isFreshCoordinate(parsed,LOCATION_MAX_AGE_MS))location=parsed;else await AsyncStorage.removeItem(LOCATION_KEY);}catch{await AsyncStorage.removeItem(LOCATION_KEY);}}
    if(cachedOrigin){try{const parsed=JSON.parse(cachedOrigin) as LiveCoordinate;if(isFreshCoordinate(parsed,REVEAL_ORIGIN_MAX_AGE_MS))revealOrigin=parsed;else await AsyncStorage.removeItem(REVEAL_ORIGIN_KEY);}catch{await AsyncStorage.removeItem(REVEAL_ORIGIN_KEY);}}
  ...
  startDrive:async()=>{
    ...
    get()._watch?.remove();let initial=get().location;if(!isUsableLiveCoordinate(initial)){try{initial=coordinate(await currentPosition());}catch(error){set({networkStatus:'gps_required',error:error instanceof Error?error.message:'Drive Mode could not acquire GPS.'});return;}}if(!initial||!shouldAcceptMovement(get().location,initial)){set({networkStatus:'gps_required',error:'Drive Mode needs a fresh accurate GPS sample before it can start.'});return;}const origin=isFreshCoordinate(get().revealOrigin,REVEAL_ORIGIN_MAX_AGE_MS)?get().revealOrigin!:initial;const shareExpiresAt=Date.now()+get().shareMinutes*60_000,driveSessionId=`drive-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;playInterfaceSound('drive');set({location:initial,isDriving:true,revealOrigin:origin,distanceKm:0,maxSpeedKph:0,startedAt:Date.now(),shareExpiresAt,error:null,_driveSessionId:driveSessionId,driveTrace:[initial],lastDriveSummary:null});await Promise.all([AsyncStorage.setItem(LOCATION_KEY,JSON.stringify(initial)),AsyncStorage.setItem(REVEAL_ORIGIN_KEY,JSON.stringify(origin))]);
    ...
    const watch=await Location.watchPositionAsync({accuracy:Location.Accuracy.BestForNavigation,timeInterval:500,distanceInterval:1},position=>{
      const next=coordinate(position);const previous=get().location;if(!shouldAcceptMovement(previous,next))return;const distance=previous&&next.accuracy!==null&&next.accuracy<=65?segmentKm(previous,next):0;
  ...
  toggleUnit:()=>set(state=>{const unit:SpeedUnit=state.unit==='mph'?'kph':'mph';void AsyncStorage.setItem(UNIT_KEY,unit);return{unit};}),
  setUnitPreference:unit=>{const next=normalizeSpeedUnit(unit);set({unit:next});void AsyncStorage.setItem(UNIT_KEY,next);},
